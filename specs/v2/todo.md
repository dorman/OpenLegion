...
The openlegion server has moved to the Effect HttpApi backend. Remaining work is
...
- plugins get global "openlegion" instance like in that post i showed
- openlegion instance has stuff like `openlegion.session.prompt()` or
  `openlegion.tool.register({...})`
