# Remove `packages/openlegion/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/openlegion/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the openlegion legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/openlegion/src/storage/db.ts` are concentrated in 22 source files:

- `packages/openlegion/src/account/repo.ts`
- `packages/openlegion/src/cli/cmd/db.ts`
- `packages/openlegion/src/cli/cmd/import.ts`
- `packages/openlegion/src/cli/cmd/stats.ts`
- `packages/openlegion/src/control-plane/workspace.ts`
- `packages/openlegion/src/index.ts`
- `packages/openlegion/src/node.ts`
- `packages/openlegion/src/permission/index.ts`
- `packages/openlegion/src/project/project.ts`
- `packages/openlegion/src/server/projectors.ts`
- `packages/openlegion/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/openlegion/src/server/shared/fence.ts`
- `packages/openlegion/src/session/message-v2.ts`
- `packages/openlegion/src/session/projectors.ts`
- `packages/openlegion/src/session/prompt.ts`
- `packages/openlegion/src/session/session.ts`
- `packages/openlegion/src/session/todo.ts`
- `packages/openlegion/src/share/share-next.ts`
- `packages/openlegion/src/storage/db.ts`
- `packages/openlegion/src/sync/index.ts`
- `packages/openlegion/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy openlegion database wrapper; `packages/openlegion/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the openlegion wrapper.
...
Status: Completed. `SyncEvent` and the openlegion projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy openlegion database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy openlegion database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy openlegion database wrapper.
...
All migration groups are complete or superseded. `packages/openlegion/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No openlegion data-migration group remains.

The previous openlegion `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate openlegion data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/openlegion`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/openlegion/src`
- `bun typecheck` from `packages/openlegion`
- Relevant package tests from `packages/openlegion`, not the repo root
