...
npm i -g openlegion-ai@latest        # o bun/pnpm/yarn
scoop install openlegion             # Windows
choco install openlegion             # Windows
brew install anomalyco/tap/openlegion # macOS y Linux (recomendado, siempre al día)
brew install openlegion              # macOS y Linux (fórmula oficial de brew, se actualiza menos)
sudo pacman -S openlegion            # Arch Linux (Stable)
paru -S openlegion-bin               # Arch Linux (Latest from AUR)
mise use -g openlegion               # cualquier sistema
nix run nixpkgs#openlegion           # o github:dorman/OpenLegion para la rama dev más reciente
```
...
OpenLegion también está disponible como aplicación de escritorio. Descárgala directamente desde la [página de releases](https://github.com/dorman/OpenLegion/releases) o desde [openlegion.dev/download](https://openlegion.dev/download).
...
1. `$OPENLEGION_INSTALL_DIR` - Directorio de instalación personalizado
2. `$XDG_BIN_DIR` - Ruta compatible con la especificación XDG Base Directory
3. `$HOME/bin` - Directorio binario estándar del usuario (si existe o se puede crear)
4. `$HOME/.openlegion/bin` - Alternativa por defecto
...
OpenLegion incluye dos agentes integrados que puedes alternar con la tecla `Tab`.
...
Más información sobre [agentes](https://openlegion.dev/docs/agents).

### Documentación

Para más información sobre cómo configurar OpenLegion, [**ve a nuestra documentación**](https://openlegion.dev/docs).

### Contribuir

Si te interesa contribuir a OpenLegion, lee nuestras [docs de contribución](./CONTRIBUTING.md) antes de enviar un pull request.

### Proyectos basados en OpenLegion

Si estás trabajando en un proyecto basado en OpenLegion y usas "openlegion" como parte del nombre, por ejemplo, "openlegion-dashboard" u "openlegion-mobile", agrega una nota en tu README para aclarar que no está hecho por el equipo de OpenLegion y que no está afiliado con nosotros de ninguna manera.

---

**Únete a nuestra comunidad** [Discord](https://discord.gg/openlegion) | [X.com](https://x.com/openlegion)
