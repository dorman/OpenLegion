...
import { createOpencode } from "@openlegion-ai/sdk"
...
`)
  ...
  const openlegion = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await openlegion.client.session.create()
    const result = await openlegion.client.session
      .prompt({
