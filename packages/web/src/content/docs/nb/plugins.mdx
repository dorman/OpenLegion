...
description: Skriv dine egne programtillegg for å utvide OpenLegion.
---

Plugins lar deg utvide OpenLegion ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til OpenLegion.
...
- `.openlegion/plugins/` - Programtillegg på prosjektnivå
...
**npm-plugins** installeres automatisk ved hjelp av Bun ved oppstart. Pakker og deres avhengigheter er bufret i `~/.cache/openlegion/node_modules/`.
...
1. Global konfigurasjon (`~/.config/openlegion/openlegion.json`)
2. Prosjektkonfigurasjon (`openlegion.json`)
3. Global plugin-katalog (`~/.config/openlegion/plugins/`)
4. Prosjektpluginkatalog (`.openlegion/plugins/`)
...
OpenLegion kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En OpenLegion SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide OpenLegion.
...
:::note
Hvis du bruker OpenLegion-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre OpenLegion fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til OpenLegion:
...
`tool`-hjelperen lager et tilpasset verktøy som OpenLegion kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for OpenLegion sammen med innebygde verktøy.
...
Nivåer: `debug`, `info`, `warn`, `error`. Se [SDK dokumentasjon](https://openlegion.dev/docs/sdk) for detaljer.
