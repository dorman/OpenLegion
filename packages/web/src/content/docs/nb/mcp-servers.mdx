...
Du kan legge til eksterne verktøy i OpenLegion ved å bruke _Model Context Protocol_ eller MCP. OpenLegion støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [OpenLegion Config](https://openlegion.dev/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="openlegion.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/openlegion`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
```jsonc title="openlegion.jsonc" {15}
{
  "$schema": "https://openlegion.dev/config.json",
  "mcp": {
    "my-local-mcp-server": {
...
OpenLegion håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil OpenLegion:
...
Hvis serveren krever autentisering, vil OpenLegion be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `openlegion mcp auth <server-name>`.
...
```json title="openlegion.json" {7-11}
{
  "$schema": "https://openlegion.dev/config.json",
  "mcp": {
    "my-oauth-server": {
...
```bash
openlegion mcp auth my-oauth-server
```
...
```bash
openlegion mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil OpenLegion lagre tokens sikkert i `~/.local/share/openlegion/mcp-auth.json`.
...
openlegion mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i OpenLegion, sammen med innebygde verktøy. Så du kan administrere dem gjennom OpenLegion-konfigurasjonen som alle andre verktøy.
...
```json title="openlegion.json" {11, 14-18}
{
  "$schema": "https://openlegion.dev/config.json",
  "mcp": {
    "my-mcp": {
...
```json title="openlegion.json" {4-8}
{
  "$schema": "https://openlegion.dev/config.json",
  "mcp": {
    "sentry": {
...
```bash
openlegion mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble OpenLegion til Sentry-kontoen din.
...
```json title="openlegion.json" {4-7}
{
  "$schema": "https://openlegion.dev/config.json",
  "mcp": {
    "context7": {
...
```json title="openlegion.json" {7-9}
{
  "$schema": "https://openlegion.dev/config.json",
  "mcp": {
    "context7": {
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver OpenLegion-samtale oppført som en separat økt.
