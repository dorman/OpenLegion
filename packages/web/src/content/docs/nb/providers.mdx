---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i OpenLegion.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenLegion bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i OpenLegion-konfigurasjonen.
...
i `~/.local/share/openlegion/auth.json`.

---

### Konfigurasjon

Du kan tilpasse leverandørene gjennom `provider`-delen i OpenLegion
konfig.
...
OpenLegion Zen er en liste over modeller levert av OpenLegion-teamet som har vært
testet og verifisert for å fungere godt med OpenLegion. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med OpenLegion Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `OpenLegion Zen` og gå til [openlegion.dev/auth](https://openlegion.dev/zen).
...
Det fungerer som alle andre leverandører i OpenLegion og er helt valgfritt å bruke.

---

## OpenLegion Go

OpenLegion Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av OpenLegion-teamet som har vært
testet og verifisert for å fungere godt med OpenLegion.

1. Kjør kommandoen `/connect` i TUI, velg `OpenLegion Go`, og gå til [openlegion.dev/auth](https://openlegion.dev/zen).
...
Slik bruker du Amazon Bedrock med OpenLegion:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører OpenLegion:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `openlegion.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i openlegion støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere openlegion til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at OpenLegion skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom OpenLegion-konfigurasjonen.
...
for å lagre token i OpenLegion auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
OpenLegion bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse OpenLegion
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`openlegion.json` fil. Det anbefales også å deaktivere øktdeling.
...
Tilpass gjennom `openlegion.json`:
...
Slik bruker du GitHub Copilot-abonnementet med OpenLegion:
...
Slik bruker du Google Vertex AI med OpenLegion:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører OpenLegion.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom OpenLegion, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [openlegion-helicone-session](https://github.com/H2Shami/openlegion-helicone-session) plugin for automatisk å logge hver OpenLegion-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver OpenLegion-samtale oppført som en separat økt.
...
Du kan konfigurere OpenLegion for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere OpenLegion for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere OpenLegion for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for OpenLegion. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/openlegion) for detaljer.
:::
...
Slik bruker du Ollama Cloud med OpenLegion:
...
3. Kopier API-nøkkelen for bruk i OpenLegion.
...
6. **Viktig**: Før du bruker skymodeller i OpenLegion, må du hente modellinformasjonen lokalt:
...
OpenLegion Zen er en liste over testede og verifiserte modeller levert av OpenLegion-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>OpenLegion Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **OpenLegion Zen**.

   ```txt
   /connect
   ```

3. Skriv inn OpenLegion API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom OpenLegion-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom OpenLegion-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med OpenLegion:
...
Du kan også tilpasse modeller gjennom OpenLegion-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med OpenLegion. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `openlegion.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar OpenLegion forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `openlegion auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk OpenLegion-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i OpenLegion-konfigurasjonen.
