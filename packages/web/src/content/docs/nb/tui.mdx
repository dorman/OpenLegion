---
title: TUI
description: Bruke openlegion-terminalbrukergrensesnittet.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenLegion gir et interaktivt terminalgrensesnitt (TUI) for å jobbe med prosjektene dine med en LLM.

Å kjøre OpenLegion starter TUI for gjeldende katalog.
...
Når du bruker OpenLegion TUI, kan du skrive `/` etterfulgt av et kommandonavn for raskt å utføre handlinger. For eksempel:
...
Legg til en leverandør til OpenLegion. Lar deg velge fra tilgjengelige leverandører og legge til deres API-nøkler.
...
Avslutt OpenLegion. _Aliaser_: `/quit`, `/q`
...
Dette er atskilt fra `openlegion.json`, som konfigurerer server-/kjøretidsoppførsel.
...
- `leader_timeout` - Styrer hvor lenge OpenLegion venter etter leader key. Standard er `2000`.
...
Bruk `OPENLEGION_TUI_CONFIG` for å laste en egendefinert TUI-konfigurasjonsbane.
...
- `sound_pack` - Sound pack-ID som skal brukes. Standard er `openlegion.default`.
