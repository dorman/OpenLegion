...
Det er to typer agenter i OpenLegion; primære agenter og underagenter.
...
OpenLegion kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
OpenLegion kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

OpenLegion kommer med to innebygde primære agenter og tre innebygde underagenter.
...
Konfigurer agenter i konfigurasjonsfilen `openlegion.json`:
...
- Per prosjekt: `.openlegion/agents/`
...
Hvis ingen temperatur er spesifisert, bruker OpenLegion modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale OpenLegion-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i OpenLegion-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [OpenLegion Zen](/docs/zen), vil du bruke `openlegion/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `openlegion models` for å se en liste over tilgjengelige modeller.
:::
...
:::tip
Har du en agent du vil dele? [Send inn en PR](https://github.com/dorman/OpenLegion).
:::
