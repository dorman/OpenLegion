...
Agentferdigheter lar OpenLegion oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
openlegion søker etter disse stedene:

- Prosjektkonfigurasjon: `.openlegion/skills/<name>/SKILL.md`
- Global konfigurasjon: `~/.config/openlegion/skills/<name>/SKILL.md`
...
For prosjektlokale stier går openlegion opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.openlegion/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.

Globale definisjoner lastes også inn fra `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` og `~/.agents/skills/*/SKILL.md`.
...
Lag `.openlegion/skills/git-release/SKILL.md` slik:
...
openlegion viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `openlegion.json`:
...
**For innebygde agenter** (i `openlegion.json`):
