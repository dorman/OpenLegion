---
title: Tjener
description: Samhandle med OpenLegion-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openlegion serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en OpenLegion-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `openlegion.local` |
...
Sett `OPENLEGION_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `openlegion` som standard, eller sett `OPENLEGION_SERVER_USERNAME` for å overstyre det. Dette gjelder både `openlegion serve` og `openlegion web`.
...
Når du kjører `openlegion` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk OpenLegion-serveren til å samhandle med OpenLegion programmatisk.
:::

Denne arkitekturen lar OpenLegion støtte flere klienter og lar deg samhandle med OpenLegion programmatisk.

Du kan kjøre `openlegion serve` for å starte en frittstående server. Hvis du har
OpenLegion TUI kjører, vil `openlegion serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av OpenLegion [IDE](/docs/ide) plugins.
...
openlegion-serveren viser følgende APIer.
