...
Możesz dodać dodatkowe informacje do openlegion za pomocą _Model Context Protocol_, w skrócie MCP. openlegion obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [openlegion Config](https://openlegion.dev/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/openlegion`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
openlegion automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, openlegion:
...
Jeśli serwer wymaga uwierzytelnienia, openlegion poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `openlegion mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji openlegion bezpieczne przechowa tokeny w `~/.local/share/openlegion/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w openlegion, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie openlegion, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć openlegion z kontem Sentry.
