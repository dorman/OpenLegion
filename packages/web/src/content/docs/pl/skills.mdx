...
Umiejętności agenta pozwalają openlegion odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
openlegion przeszukuje te lokalizacje:

- Project config: `.openlegion/skills/<name>/SKILL.md`
- Global config: `~/.config/openlegion/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu openlegion przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.openlegion/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.

Ładowane są także definicje globalne z `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Utwórz `.openlegion/skills/git-release/SKILL.md` w ten sposób:
...
openlegion wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `openlegion.json`:
...
**Dla agentów wbudowanych** (w `openlegion.json`):
