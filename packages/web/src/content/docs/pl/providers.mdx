---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w openlegion.
---

import config from "../../../../config.mjs"
export const console = config.console

openlegion używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji openlegion.
...
w `~/.local/share/openlegion/auth.json`.

---

### Konfiguracja

Możesz dostosować dostawców za pomocą sekcji `provider` w swoim openlegion
config.
...
OpenLegion Zen to lista modeli dostarczonych przez zespół openlegion, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z openlegion. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od OpenLegion Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenLegion Zen` i przejdź do [openlegion.dev/auth](https://openlegion.dev/zen).
...
Działa jak każdy inny dostawca w openlegion i jest całkowicie opcjonalny w użyciu.

---

## OpenLegion Go

OpenLegion Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół openlegion, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z openlegion.

1. Uruchom polecenie `/connect` w TUI, wybierz `OpenLegion Go` i przejdź do [openlegion.dev/auth](https://openlegion.dev/zen).
...
Aby używać Amazon Bedrock z openlegion:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania openlegion:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `openlegion.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w openlegion nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować openlegion do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod openlegion działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji openlegion.
...
openlegion używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować openlegion
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`openlegion.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Aby korzystać z subskrypcji GitHub Copilot z kodem openlegion:
...
Aby używać Google Vertex AI z openlegion:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania openlegion.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą openlegion, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [openlegion-helicone-session](https://github.com/H2Shami/openlegion-helicone-session), aby automatycznie rejestrować każdą konwersację openlegion jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja openlegion będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować openlegion tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować openlegion tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować openlegion tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem openlegion. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/openlegion).
:::
...
Aby korzystać z Ollama Cloud z openlegion:
...
3. Skopiuj klucz API do użycia w openlegion.
...
6. **Ważne**: Przed użyciem modeli chmurowych w openlegion musisz pobrać informacje o modelu lokalnie:
...
OpenLegion Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół openlegion. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>OpenLegion Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **OpenLegion Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API openlegion.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji openlegion.
...
5. Możesz je także dostosować za pomocą konfiguracji openlegion. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z openlegion:
...
Możesz także dostosować modele za pomocą konfiguracji openlegion. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z openlegion. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `openlegion.json` w katalogu projektu:
...
Pola `limit` pozwalają openlegion zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `openlegion auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację openlegion i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji openlegion.
