---
title: Serwer
description: Interakcja z serwerem openlegion za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `openlegion serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient openlegion.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `openlegion.local` |
...
Ustaw `OPENLEGION_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `openlegion` lub ustaw `OPENLEGION_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `openlegion serve`, jak i `openlegion web`.
...
Kiedy uruchomisz `openlegion`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera openlegion do programowej interakcji z openlegion.
:::

Ta architektura umożliwia obsługę wielu klientów przez openlegion i programową interakcję z kodem otwartym.

Możesz uruchomić `openlegion serve`, aby uruchomić samodzielny serwer. Jeśli masz
openlegion TUI działa, `openlegion serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki openlegion [IDE](/docs/ide).
...
Serwer openlegion udostępnia następujące interfejsy API.
