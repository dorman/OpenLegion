...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. OpenLegion posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w OpenLegion.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `OPENLEGION_EXPERIMENTAL_LSP_TOOL=true` (lub `OPENLEGION_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy OpenLegion lub gdy zmienna środowiskowa `OPENLEGION_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu OpenLegion:
