---
title: Serwery LSP
description: OpenLegion integruje się z serwerami LSP.
---

OpenLegion może integrować się z serwerami Language Server Protocol (LSP), aby używać diagnostyki jako informacji zwrotnej dla agenta.

---

## Wbudowane

OpenLegion posiada kilka wbudowanych serwerów LSP dla następujących języków:
...
:::note
Możesz wyłączyć automatyczne pobieranie serwerów LSP, ustawiając zmienną środowiskową `OPENLEGION_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Gdy LSP jest włączony i OpenLegion otwiera plik:
...
Możesz włączać i dostosowywać serwery LSP poprzez sekcję `lsp` w konfiguracji OpenLegion.
