...
Dzięki openlegion możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie openlegion używa naszego własnego motywu `openlegion`.
...
openlegion ma kilka wbudowanych motywów.
...
- Chcą, aby OpenLegion pasował do wyglądu ich terminala
...
openlegion obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
2. **Katalog konfiguracji użytkownika** - Zdefiniowany w `~/.config/openlegion/themes/*.json` lub `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.openlegion/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.openlegion/themes/*.json`
