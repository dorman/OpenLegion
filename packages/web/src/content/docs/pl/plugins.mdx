---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć openlegion.
---

Wtyczki umożliwiające wyłączenie openlegion poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania openlegion.
...
- `.openlegion/plugins/` - Wtyczki na poziomie projektu
- `~/.config/openlegion/plugins/` - Wtyczki globalne
...
**wtyczki npm** są instalowane automatycznie przy użyciu Bun podczas uruchamiania. Pakiety i ich zależności są buforowane w `~/.cache/openlegion/node_modules/`.
...
1. Konfiguracja globalna (`~/.config/openlegion/openlegion.json`)
2. Konfiguracja projektu (`openlegion.json`)
3. Globalny katalog wtyczek (`~/.config/openlegion/plugins/`)
4. Katalog wtyczek projektu (`.openlegion/plugins/`)
...
openlegion uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z openlegion do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do openlegion.
...
:::note
Jeśli korzystasz z aplikacji komputerowej openlegion, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj openlegion czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do openlegion:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod openlegion. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
...
Poziomy: `debug`, `info`, `warn`, `error`. Aby uzyskać szczegółowe informacje, zobacz [dokumentację pakietu SDK](https://openlegion.dev/docs/sdk).
