...
OpenLegion Go to niskokosztowa subskrypcja — **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc** — która zapewnia niezawodny dostęp do popularnych otwartych modeli do kodowania.

Go działa jak każdy inny dostawca w OpenLegion. Subskrybujesz OpenLegion Go i
...
OpenLegion Go daje Ci dostęp do tych modeli za **5 $ za pierwszy miesiąc**, a następnie **10 $/miesiąc**.

---

## Jak to działa

OpenLegion Go działa jak każdy inny dostawca w OpenLegion.

1. Logujesz się do **<a href={console}>OpenLegion Zen</a>**, subskrybujesz Go i
   kopiujesz swój klucz API.
2. Uruchamiasz komendę `/connect` w TUI, wybierasz `OpenLegion Go` i wklejasz
   swój klucz API.
3. Uruchom `/models` w TUI, aby zobaczyć listę modeli dostępnych w ramach Go.

:::note
Tylko jeden członek na obszar roboczy (workspace) może zasubskrybować OpenLegion Go.
:::
...
OpenLegion Go zawiera następujące limity:
...
[ID modelu](/docs/config/#models) w Twojej konfiguracji OpenLegion
używa formatu `openlegion-go/<model-id>`. Na przykład dla Kimi K2.6 należy użyć
`openlegion-go/kimi-k2.6` w swojej konfiguracji.
...
Stworzyliśmy OpenLegion Go, aby:
...
4. Zapewnić **brak ograniczeń (no lock-in)**, pozwalając na korzystanie z dowolnego innego dostawcy wraz z OpenLegion.
