---
title: Server
description: Interact with openlegion server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `openlegion serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an openlegion client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `openlegion.local` |
...
Set `OPENLEGION_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `openlegion`, or set `OPENLEGION_SERVER_USERNAME` to override it. This applies to both `openlegion serve` and `openlegion web`.
...
When you run `openlegion` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the openlegion server to interact with openlegion programmatically.
:::

This architecture lets openlegion support multiple clients and allows you to interact with openlegion programmatically.

You can run `openlegion serve` to start a standalone server. If you have the
openlegion TUI running, `openlegion serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the OpenLegion [IDE](/docs/ide) plugins.
...
The openlegion server exposes the following APIs.
