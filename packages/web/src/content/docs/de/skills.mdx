...
Agent Skills erlauben OpenLegion, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
OpenLegion sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft OpenLegion vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.openlegion/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.

Globale Definitionen kommen zusaetzlich aus `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` und `~/.agents/skills/*/SKILL.md`.
...
Erstelle `.openlegion/skills/git-release/SKILL.md` zum Beispiel so:
...
OpenLegion listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `openlegion.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `openlegion.json`):
