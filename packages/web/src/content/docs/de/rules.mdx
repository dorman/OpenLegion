---
title: Regeln
description: Lege eigene Anweisungen fuer openlegion fest.
---

Du kannst openlegion ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in openlegion den Befehl `/init` aus.
...
So kann openlegion sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht openlegion sie zu erweitern.
...
openlegion kann `AGENTS.md` aus mehreren Orten lesen.
...
Du kannst auch globale Regeln in `~/.config/openlegion/AGENTS.md` speichern.
Diese gelten in allen openlegion-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt OpenLegion dessen Dateikonventionen als Fallback:

- **Projektregeln**: `CLAUDE.md` im Projekt (wenn keine `AGENTS.md` existiert)
- **Globale Regeln**: `~/.claude/CLAUDE.md` (wenn keine `~/.config/openlegion/AGENTS.md` existiert)
...
Beim Start sucht openlegion in dieser Reihenfolge nach Regeldateien:

1. **Lokale Dateien** beim Hochlaufen ab dem aktuellen Verzeichnis (`AGENTS.md`, `CLAUDE.md`)
2. **Globale Datei** unter `~/.config/openlegion/AGENTS.md`
...
Du kannst zusaetzliche Anweisungsdateien in deiner `openlegion.json` oder in `~/.config/openlegion/openlegion.json` angeben.
...
openlegion parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit openlegion.json

Empfohlen ist das `instructions`-Feld in der `openlegion.json`:
...
Du kannst openlegion explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `openlegion.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
