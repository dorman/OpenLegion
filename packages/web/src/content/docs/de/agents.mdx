...
Es gibt zwei Arten von Agenten in OpenLegion; Primäragenten und Subagenten.
...
OpenLegion verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `openlegion.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet OpenLegion modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale OpenLegion-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer OpenLegion-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [OpenLegion Zen](/docs/zen) verwenden, würden Sie `openlegion/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `openlegion models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
...
:::tip
Haben Sie einen Agenten, den Sie gerne teilen möchten? [Reichen Sie einen PR ein](https://github.com/dorman/OpenLegion).
:::
