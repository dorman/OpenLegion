...
Wenn OpenLegion Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openlegion\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `openlegion --log-level DEBUG`.

---

## Speicher

openlegion speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/openlegion/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\openlegion`
...
OpenLegion Desktop startet im Hintergrund einen lokalen OpenLegion-Server (`openlegion-cli`-Sidecar).
...
- Nur macOS: `OpenLegion`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/openlegion/openlegion.jsonc` (oder `~/.config/openlegion/openlegion.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/openlegion/openlegion.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openlegion\openlegion.jsonc`
...
OpenLegion kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/openlegion/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\openlegion\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit OpenLegion ihn neu aufbauen kann.

1. Beende OpenLegion Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/openlegion`
- **Linux**: loesche `~/.cache/openlegion` (oder fuehre aus: `rm -rf ~/.cache/openlegion`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\openlegion`

3. Starte OpenLegion Desktop neu.

---

### Server-Verbindungsprobleme beheben

OpenLegion Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `openlegion.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `OPENLEGION_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `OPENLEGION_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt OpenLegion Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit OpenLegion-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

OpenLegion Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer OpenLegion in den OS-Einstellungen aktiviert sind, und
...
1. Beende OpenLegion Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von OpenLegion Desktop):

- `openlegion.settings.dat` (desktop default server URL)
- `openlegion.global.dat` und `openlegion.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit OpenLegion hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `openlegion models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\openlegion`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. openlegion installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\openlegion`

2. Starte openlegion neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt openlegion, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
openlegion erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
