...
Sie können externe Tools zu OpenLegion hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. OpenLegion unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [OpenLegion Config](https://openlegion.dev/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/openlegion`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
OpenLegion übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird OpenLegion Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von OpenLegion beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `openlegion mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert OpenLegion die Token sicher in `~/.local/share/openlegion/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in OpenLegion verfügbar. Sie können sie also wie jedes andere Tool über die OpenLegion-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und OpenLegion mit Ihrem Sentry-Konto zu verbinden.
