---
title: Plugins
description: Schreiben Sie Ihre eigenen Plugins, um OpenLegion zu erweitern.
---

Mit Plugins können Sie OpenLegion erweitern, indem Sie verschiedene Ereignisse einbinden und das Verhalten anpassen. Sie können Plugins erstellen, um neue Funktionen hinzuzufügen, externe Dienste zu integrieren oder das Standardverhalten von OpenLegion zu ändern.
...
- `.openlegion/plugins/` – Plugins auf Projektebene
- `~/.config/openlegion/plugins/` – Globale Plugins
...
**NPM-Plugins** werden beim Start automatisch mit Bun installiert. Pakete und ihre Abhängigkeiten werden in `~/.cache/openlegion/node_modules/` zwischengespeichert.
...
1. Globale Konfiguration (`~/.config/openlegion/openlegion.json`)
2. Projektkonfiguration (`openlegion.json`)
3. Globales Plugin-Verzeichnis (`~/.config/openlegion/plugins/`)
4. Projekt-Plugin-Verzeichnis (`.openlegion/plugins/`)
...
OpenLegion führt beim Start `bun install` aus, um diese zu installieren. Ihre Plugins und Tools können sie dann importieren.
...
- `client`: Ein OpenLegion-SDK-Client für die Interaktion mit AI.
...
Hier sind einige Beispiele für Plugins, mit denen Sie OpenLegion erweitern können.
...
:::note
Wenn Sie die OpenLegion-Desktop-App verwenden, kann diese automatisch Systembenachrichtigungen senden, wenn eine Antwort bereit ist oder wenn in einer Sitzung ein Fehler auftritt.
:::
...
Verhindern Sie, dass OpenLegion `.env`-Dateien liest:
...
Plugins können OpenLegion auch benutzerdefinierte Tools hinzufügen:
...
Der `tool`-Helfer erstellt ein benutzerdefiniertes Tool, das OpenLegion aufrufen kann. Es verwendet eine Zod-Schemafunktion und gibt eine Tooldefinition zurück mit:
...
Ihre benutzerdefinierten Tools stehen neben den integrierten Tools für OpenLegion zur Verfügung.
...
Ebenen: `debug`, `info`, `warn`, `error`. Weitere Informationen finden Sie unter [SDK documentation](https://openlegion.dev/docs/sdk).
