...
Mit OpenLegion kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet OpenLegion das `openlegion`-Theme.
...
OpenLegion liefert mehrere eingebaute Themes mit.
...
- OpenLegion optisch ans Terminal anpassen wollen
...
OpenLegion unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
2. **Benutzer-Config-Verzeichnis** - Definiert in `~/.config/openlegion/themes/*.json` oder `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.openlegion/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.openlegion/themes/*.json`
