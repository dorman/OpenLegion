...
Erstellen Sie `.openlegion/commands/test.md`:

```md title=".openlegion/commands/test.md"
---
...
Sie können benutzerdefinierte Befehle über die OpenLegion-Konfiguration hinzufügen oder indem Sie Markdown-Dateien im Verzeichnis `commands/` erstellen.

---

### JSON

Verwenden Sie die Option `command` in Ihrem OpenLegion [config](/docs/config):

```json title="openlegion.jsonc" {4-12}
{
...
- Global: `~/.config/openlegion/commands/`
- Pro Projekt: `.openlegion/commands/`

```markdown title="~/.config/openlegion/commands/test.md"
---
description: Run tests with coverage
agent: build
model: anthropic/claude-3-5-sonnet-20241022
...
```md title=".openlegion/commands/component.md"
---
...
```md title=".openlegion/commands/create-file.md"
---
...
```md title=".openlegion/commands/analyze-coverage.md"
---
...
```md title=".openlegion/commands/review-changes.md"
---
...
```md title=".openlegion/commands/review-component.md"
---
...
OpenLegion enthält mehrere integrierte Befehle wie `/init`, `/undo`, `/redo`, `/share`, `/help`; [mehr erfahren](/docs/tui#commands).
