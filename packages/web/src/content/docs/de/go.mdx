...
OpenLegion Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in OpenLegion. Du abonnierst OpenLegion Go und
erhältst deinen API-Key. Es ist **völlig optional** und du musst es nicht nutzen, um
OpenLegion zu verwenden.
...
OpenLegion Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

OpenLegion Go funktioniert wie jeder andere Provider in OpenLegion.

1. Du meldest dich bei **<a href={console}>OpenLegion Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `OpenLegion Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann OpenLegion Go abonnieren.
:::
...
OpenLegion Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner OpenLegion Config verwendet das Format `openlegion-go/<model-id>`. Für Kimi K2.6 würdest du beispielsweise `openlegion-go/kimi-k2.6` in deiner Config verwenden.
...
Wir haben OpenLegion Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit OpenLegion zu nutzen.
