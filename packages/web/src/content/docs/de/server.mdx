---
title: Server
description: Nutze den openlegion-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `openlegion serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein openlegion-Client nutzen kann.
...
Setze `OPENLEGION_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `openlegion`, kann aber mit `OPENLEGION_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `openlegion serve` und `openlegion web`.
...
Wenn du `openlegion` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den openlegion-Server, um openlegion programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `openlegion serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `openlegion serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die OpenLegion-[IDE](/docs/ide)-Plugins.
...
Der openlegion-Server stellt folgende APIs bereit.
