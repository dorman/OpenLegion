---
title: Server
description: Interagisci con il server openlegion via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Il comando `openlegion serve` avvia un server HTTP headless che espone un endpoint OpenAPI utilizzabile da un client openlegion.
...
| `--mdns-domain` | Nome di dominio personalizzato mDNS     | `openlegion.local` |
...
Imposta `OPENLEGION_SERVER_PASSWORD` per proteggere il server con HTTP basic auth. Lo username predefinito e `openlegion`, oppure imposta `OPENLEGION_SERVER_USERNAME` per sovrascriverlo. Questo vale sia per `openlegion serve` sia per `openlegion web`.
...
Quando esegui `openlegion` avvia una TUI e un server. La TUI e il client che parla col server. Il server espone un endpoint con specifica OpenAPI 3.1. Questo endpoint viene anche usato per generare un [SDK](/docs/sdk).

:::tip
Usa il server openlegion per interagire con openlegion in modo programmatico.
:::

Questa architettura permette a openlegion di supportare piu client e di essere usato in modo programmatico.

Puoi eseguire `openlegion serve` per avviare un server standalone. Se la TUI di openlegion e gia in esecuzione, `openlegion serve` avviera un nuovo server.
...
L'endpoint [`/tui`](#tui) puo essere usato per pilotare la TUI tramite il server. Per esempio, puoi precompilare o eseguire un prompt. Questa configurazione e usata dai plugin [IDE](/docs/ide) di OpenLegion.
...
Il server openlegion espone le seguenti API.
