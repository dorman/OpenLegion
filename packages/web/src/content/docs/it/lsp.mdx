---
title: Server LSP
description: OpenLegion si integra con i tuoi server LSP.
---

OpenLegion può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

OpenLegion include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `OPENLEGION_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e openlegion apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di openlegion.
