...
Le skill degli agenti permettono a OpenLegion di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
OpenLegion cerca in queste posizioni:

- Config di progetto: `.openlegion/skills/<name>/SKILL.md`
- Config globale: `~/.config/openlegion/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, OpenLegion risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.openlegion/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.

Le definizioni globali vengono caricate anche da `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crea `.openlegion/skills/git-release/SKILL.md` cosi':
...
OpenLegion elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `openlegion.json`:
...
**Per agenti integrati** (in `openlegion.json`):
