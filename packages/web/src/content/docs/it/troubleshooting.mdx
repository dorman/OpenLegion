...
Per diagnosticare problemi con OpenLegion, inizia controllando i log e i dati locali che salva su disco.
...
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\openlegion\log`

I file di log hanno nomi con timestamp (ad es. `2025-01-09T123456.log`) e vengono conservati i 10 file di log piu recenti.

Puoi impostare il livello di log con l'opzione a riga di comando `--log-level` per ottenere informazioni di debug piu dettagliate. Per esempio: `openlegion --log-level DEBUG`.

---

## Archiviazione

openlegion salva i dati delle sessioni e altri dati dell'applicazione su disco in:

- **macOS/Linux**: `~/.local/share/openlegion/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.local\share\openlegion`
...
OpenLegion Desktop esegue in background un server locale di OpenLegion (il sidecar `openlegion-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
- Solo macOS: menu `OpenLegion` -> **Reload Webview** (utile se l'interfaccia e vuota o bloccata).
...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/openlegion/openlegion.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\openlegion\openlegion.jsonc`
...
OpenLegion puo anche caricare plugin locali dal disco. Spostali temporaneamente altrove (o rinomina la cartella) e riavvia l'app desktop:

- **Plugin globali**
  - **macOS/Linux**: `~/.config/openlegion/plugins/`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\openlegion\plugins`
...
Se disabilitare i plugin non aiuta (o l'installazione di un plugin e bloccata), svuota la cache in modo che OpenLegion possa ricostruirla.

1. Chiudi completamente OpenLegion Desktop.
2. Elimina la directory della cache:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/openlegion`
- **Linux**: elimina `~/.cache/openlegion` (oppure esegui `rm -rf ~/.cache/openlegion`)
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.cache\openlegion`

3. Riavvia OpenLegion Desktop.

---

### Risolvi problemi di connessione al server

OpenLegion Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Se il tuo `openlegion.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.

#### Controlla le variabili d'ambiente

Se hai `OPENLEGION_PORT` impostato nell'ambiente, l'app desktop provera a usare quella porta per il server locale.

- Rimuovi `OPENLEGION_PORT` (o scegli una porta libera) e riavvia.
...
Su Windows, OpenLegion Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.

---

### Windows: problemi generali di prestazioni

Se riscontri prestazioni lente, problemi di accesso ai file o problemi del terminale su Windows, prova a usare [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL fornisce un ambiente Linux che funziona in modo piu fluido con le funzionalita di OpenLegion.

---

### Notifiche non visualizzate

OpenLegion Desktop mostra le notifiche di sistema solo quando:

- le notifiche sono abilitate per OpenLegion nelle impostazioni del sistema operativo, e
...
1. Chiudi OpenLegion Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app OpenLegion Desktop):

- `openlegion.settings.dat` (desktop default server URL)
- `openlegion.global.dat` e `openlegion.workspace.*.dat` (stato dell'interfaccia come server/progetti recenti)
...
Se riscontri problemi con OpenLegion:
...
Per capire a quali modelli hai accesso, esegui `openlegion models`
...
2. Se il problema persiste, prova a cancellare la configurazione salvata:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.local\share\openlegion`
...
Se incontri errori nelle chiamate API, potrebbe dipendere da pacchetti provider non aggiornati. openlegion installa dinamicamente i pacchetti provider (OpenAI, Anthropic, Google, ecc.) quando servono e li mette in cache localmente.
...
1. Svuota la cache dei pacchetti provider:
   ...
   Su Windows, premi `WIN+R` ed elimina: `%USERPROFILE%\.cache\openlegion`

2. Riavvia openlegion per reinstallare i pacchetti provider piu recenti

Questo forzera openlegion a scaricare le versioni piu recenti dei pacchetti provider, cosa che spesso risolve problemi di compatibilita con parametri dei modelli e cambiamenti delle API.
...
openlegion rilevera se stai usando Wayland e preferira `wl-clipboard`; altrimenti provera a trovare gli strumenti per gli appunti nell'ordine: `xclip` e `xsel`.
