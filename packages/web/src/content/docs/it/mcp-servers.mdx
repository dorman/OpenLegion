...
Puoi aggiungere strumenti esterni a OpenLegion usando il _Model Context Protocol_, o MCP. OpenLegion supporta sia server locali sia remoti.
...
Puoi definire server MCP nella tua [OpenLegion Config](https://openlegion.dev/docs/config/) sotto `mcp`. Aggiungi ogni MCP con un nome univoco. Puoi riferirti a quell'MCP per nome quando fai prompt all'LLM.
...
Le organizzazioni possono fornire server MCP predefiniti tramite l'endpoint `.well-known/openlegion`. Questi server possono essere disabilitati di default, lasciando agli utenti la possibilita' di fare opt-in su quelli necessari.
...
OpenLegion gestisce automaticamente l'autenticazione OAuth per i server MCP remoti. Quando un server richiede autenticazione, OpenLegion:
...
Se il server richiede autenticazione, OpenLegion ti chiedera' di autenticarti la prima volta che provi a usarlo. In caso contrario, puoi [attivare manualmente il flusso](#authenticating) con `openlegion mcp auth <server-name>`.
...
Il comando `mcp auth` aprira' il browser per l'autorizzazione. Dopo l'autorizzazione, OpenLegion memorizzera' i token in modo sicuro in `~/.local/share/openlegion/mcp-auth.json`.
...
I tuoi MCP sono disponibili come strumenti in OpenLegion insieme agli strumenti integrati. Quindi puoi gestirli tramite la configurazione OpenLegion come qualunque altro strumento.
...
Questo aprira' una finestra del browser per completare il flusso OAuth e collegare OpenLegion al tuo account Sentry.
