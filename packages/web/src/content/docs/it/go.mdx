...
OpenLegion Go è un abbonamento a basso costo — **5 $ per il primo mese**, poi **10 $/mese** — che ti offre un accesso affidabile ai popolari modelli di programmazione aperti.

Go funziona come qualsiasi altro provider in OpenLegion. Ti abboni a OpenLegion Go e
ottieni la tua chiave API. È **completamente facoltativo** e non hai bisogno di usarlo per
utilizzare OpenLegion.
...
OpenLegion Go ti dà accesso a questi modelli a **5 $ per il primo mese**, poi a **10 $/mese**.

---

## Come funziona

OpenLegion Go funziona come qualsiasi altro provider in OpenLegion.

1. Accedi a **<a href={console}>OpenLegion Zen</a>**, ti abboni a Go e
   copi la tua chiave API.
2. Esegui il comando `/connect` nella TUI, selezioni `OpenLegion Go` e incolli
   la tua chiave API.
3. Esegui `/models` nella TUI per vedere l'elenco dei modelli disponibili tramite Go.

:::note
Solo un membro per workspace può abbonarsi a OpenLegion Go.
:::
...
OpenLegion Go include i seguenti limiti:
...
Il [model id](/docs/config/#models) nella tua OpenLegion config
utilizza il formato `openlegion-go/<model-id>`. Ad esempio, per Kimi K2.6, useresti
`openlegion-go/kimi-k2.6` nella tua configurazione.
...
Abbiamo creato OpenLegion Go per:
...
4. Non avere **nessun lock-in**, permettendoti di utilizzare anche qualsiasi altro provider con OpenLegion.
