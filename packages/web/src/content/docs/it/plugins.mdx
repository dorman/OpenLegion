---
title: Plugin
description: Scrivi plugin per estendere OpenLegion.
---

I plugin ti permettono di estendere OpenLegion agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di OpenLegion.
...
- `.openlegion/plugins/` - plugin a livello progetto
- `~/.config/openlegion/plugins/` - plugin globali
...
I **plugin npm** vengono installati automaticamente usando Bun all'avvio. I pacchetti e le loro dipendenze vengono cache-ati in `~/.cache/openlegion/node_modules/`.
...
1. Config globale (`~/.config/openlegion/openlegion.json`)
2. Config di progetto (`openlegion.json`)
3. Directory plugin globale (`~/.config/openlegion/plugins/`)
4. Directory plugin di progetto (`.openlegion/plugins/`)
...
OpenLegion esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di openlegion per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere openlegion.
...
:::note
Se usi l'app desktop di OpenLegion, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a openlegion di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a openlegion:
...
L'helper `tool` crea uno strumento personalizzato che openlegion può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in openlegion insieme agli strumenti integrati.
...
Livelli: `debug`, `info`, `warn`, `error`. Vedi la [documentazione SDK](https://openlegion.dev/docs/sdk) per i dettagli.
