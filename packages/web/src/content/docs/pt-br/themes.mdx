...
Com o openlegion, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o openlegion usa nosso próprio tema `openlegion`.
...
O openlegion vem com vários temas embutidos.
...
- Querem que o openlegion corresponda à aparência do seu terminal
...
O openlegion suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
2. **Diretório de configuração do usuário** - Definido em `~/.config/openlegion/themes/*.json` ou `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Diretório raiz do projeto** - Definido em `<project-root>/.openlegion/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.openlegion/themes/*.json`
