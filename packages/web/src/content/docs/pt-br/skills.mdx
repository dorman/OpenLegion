...
As habilidades do agente permitem que o openlegion descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O openlegion pesquisa nesses locais:

- Configuração do projeto: `.openlegion/skills/<name>/SKILL.md`
- Configuração global: `~/.config/openlegion/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o openlegion sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.openlegion/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.

As definições globais também são carregadas de `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` e `~/.agents/skills/*/SKILL.md`.
...
Crie `.openlegion/skills/git-release/SKILL.md` assim:
...
O openlegion lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `openlegion.json`:
...
**Para agentes embutidos** (em `openlegion.json`):
