---
title: Plugins
description: Escreva seus próprios plugins para estender o openlegion.
---

Plugins permitem que você estenda o openlegion conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do openlegion.
...
- `.openlegion/plugins/` - Plugins em nível de projeto
- `~/.config/openlegion/plugins/` - Plugins globais
...
**Plugins npm** são instalados automaticamente usando Bun na inicialização. Pacotes e suas dependências são armazenados em cache em `~/.cache/openlegion/node_modules/`.
...
1. Configuração global (`~/.config/openlegion/openlegion.json`)
2. Configuração do projeto (`openlegion.json`)
3. Diretório de plugins global (`~/.config/openlegion/plugins/`)
4. Diretório de plugins do projeto (`.openlegion/plugins/`)
...
O openlegion executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do openlegion para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o openlegion.
...
:::note
Se você estiver usando o aplicativo desktop openlegion, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o openlegion de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao openlegion:
...
O helper `tool` cria uma ferramenta personalizada que o openlegion pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o openlegion junto com as ferramentas integradas.
...
Níveis: `debug`, `info`, `warn`, `error`. Veja a [documentação do SDK](https://openlegion.dev/docs/sdk) para detalhes.
