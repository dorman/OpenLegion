...
description: O openlegion integra-se aos seus servidores LSP.
---

O openlegion pode se integrar a servidores Language Server Protocol (LSP) para usar diagnósticos como feedback para o agente.

---

## Integrados

O openlegion vem com vários servidores LSP integrados para linguagens populares:
...
:::note
Você pode desabilitar os downloads automáticos do servidor LSP definindo a variável de ambiente `OPENLEGION_DISABLE_LSP_DOWNLOAD` como `true`.
:::
...
Quando o LSP está habilitado e o openlegion abre um arquivo, ele:
...
Você pode habilitar e personalizar os servidores LSP através da seção `lsp` na sua configuração do openlegion.
