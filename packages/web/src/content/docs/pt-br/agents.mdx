...
Existem dois tipos de agentes no openlegion; agentes primários e subagentes.
...
openlegion vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
OpenLegion vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

OpenLegion vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do OpenLegion, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `openlegion.json`:
...
- Global: `~/.config/openlegion/agents/`
- Por projeto: `.openlegion/agents/`

```markdown title="~/.config/openlegion/agents/review.md"
---
...
Se nenhuma temperatura for especificada, o openlegion usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do openlegion quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do openlegion usa o formato `provider/model-id`. Por exemplo, se você estiver usando [OpenLegion Zen](/docs/zen), você usaria `openlegion/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="openlegion.json" {3-6,9-12}
{
  "$schema": "https://openlegion.dev/config.json",
  "tools": {
    "write": true,
...
```json title="openlegion.json" {3-5,8-10}
{
  "$schema": "https://openlegion.dev/config.json",
  "permission": {
    "edit": "deny"
...
```json title="openlegion.json" {6,7}
{
  "agent": {
    "deep-thinker": {
...
:::tip
Execute `openlegion models` para ver uma lista dos modelos disponíveis.
:::
...
:::tip
Você tem um agente que gostaria de compartilhar? [Envie um PR](https://github.com/dorman/OpenLegion).
:::
...
```markdown title="~/.config/openlegion/agents/docs-writer.md"
---
...
```markdown title="~/.config/openlegion/agents/security-auditor.md"
---
