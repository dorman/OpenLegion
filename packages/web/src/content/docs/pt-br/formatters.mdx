---
title: Formatadores
description: O openlegion usa formatadores específicos de linguagem.
---

O openlegion formata automaticamente arquivos após serem escritos ou editados usando formatadores específicos de linguagem. Isso garante que o código gerado siga os estilos de código do seu projeto.

---

## Integrados

O openlegion vem com vários formatadores integrados para linguagens e frameworks populares. Abaixo está uma lista dos formatadores, extensões de arquivo suportadas e comandos ou opções de configuração necessárias.
...
Portanto, se seu projeto tiver `prettier` em seu `package.json`, o openlegion o usará automaticamente.

---

## Como funciona

Quando o openlegion escreve ou edita um arquivo, ele:
...
Você pode personalizar os formatadores através da seção `formatter` em sua configuração do openlegion.
