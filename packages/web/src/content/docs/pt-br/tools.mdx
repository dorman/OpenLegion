...
As ferramentas permitem que o LLM execute ações em sua base de código. O openlegion vem com um conjunto de ferramentas integradas, mas você pode estendê-lo com [ferramentas personalizadas](/docs/custom-tools) ou [servidores MCP](/docs/mcp-servers).
...
Aqui estão todas as ferramentas integradas disponíveis no openlegion.
...
:::note
Esta ferramenta está disponível apenas quando `OPENLEGION_EXPERIMENTAL_LSP_TOOL=true` (ou `OPENLEGION_EXPERIMENTAL=true`).
:::
...
:::note
Esta ferramenta está disponível apenas ao usar o provedor openlegion ou quando a variável de ambiente `OPENLEGION_ENABLE_EXA` está definida como qualquer valor verdadeiro (por exemplo, `true` ou `1`).

Para habilitar ao iniciar o openlegion:
