---
title: Servidor
description: Interaja com o servidor openlegion via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `openlegion serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente openlegion pode usar.
...
```bash
openlegion serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `openlegion.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
openlegion serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `OPENLEGION_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `openlegion`, ou defina `OPENLEGION_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `openlegion serve` quanto ao `openlegion web`.

```bash
OPENLEGION_SERVER_PASSWORD=your-password openlegion serve
```
...
Quando você executa `openlegion`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor openlegion para interagir com o openlegion programaticamente.
:::

Essa arquitetura permite que o openlegion suporte múltiplos clientes e permite que você interaja com o openlegion programaticamente.

Você pode executar `openlegion serve` para iniciar um servidor autônomo. Se você tiver o TUI do openlegion em execução, `openlegion serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do openlegion [IDE](/docs/ide).
...
O servidor openlegion expõe as seguintes APIs.
