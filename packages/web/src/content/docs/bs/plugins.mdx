---
title: Dodaci
description: Napišite vlastite dodatke za proširenje OpenLegion.
---

Dodaci vam omogućavaju da proširite OpenLegion spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja OpenLegion.
...
- `.openlegion/plugins/` - Dodaci na nivou projekta
- `~/.config/openlegion/plugins/` - Globalni dodaci
...
**npm dodaci** se instaliraju automatski pomoću Bun pri pokretanju. Paketi i njihove zavisnosti su keširani u `~/.cache/openlegion/node_modules/`.
...
1. Globalna konfiguracija (`~/.config/openlegion/openlegion.json`)
2. Konfiguracija projekta (`openlegion.json`)
3. Globalni direktorij dodataka (`~/.config/openlegion/plugins/`)
4. Direktorij dodataka projekta (`.openlegion/plugins/`)
...
OpenLegion pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje OpenLegion.
...
Spriječite openlegion da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u openlegion:
...
Pomoćnik `tool` kreira prilagođeni alat koji openlegion može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za openlegion zajedno sa ugrađenim alatima.
...
Nivoi su: `debug`, `info`, `warn`, `error`. Pogledajte [SDK dokumentaciju](https://openlegion.dev/docs/sdk) za detalje.
