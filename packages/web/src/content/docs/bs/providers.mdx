---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u OpenLegion.
---

import config from "../../../../config.mjs"
export const console = config.console

OpenLegion koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj OpenLegion konfiguraciji.
...
u `~/.local/share/openlegion/auth.json`.

---

### Konfiguracija

Možete prilagoditi dobavljače putem odjeljka `provider` u vašem OpenLegion
config.
...
OpenLegion Zen je lista modela koje je obezbedio OpenLegion tim koji su bili
testirano i potvrđeno da dobro radi sa OpenLegion. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa OpenLegion Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite openlegion i idite na [openlegion.dev/auth](https://openlegion.dev/auth).
...
Radi kao i svaki drugi provajder u OpenLegion i potpuno je opcionalan za korištenje.

---

## OpenLegion Go

OpenLegion Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža OpenLegion tim i koji su testirani i verificirani da dobro rade s OpenLegion-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `OpenLegion Go` i idite na [openlegion.dev/auth](https://openlegion.dev/zen).
...
Da biste koristili Amazon Bedrock s OpenLegion:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete openlegion:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `openlegion.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u OpenLegion.
:::
...
Možete konfigurirati openlegion za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi openlegion ispravno radio.
   :::
...
Također možete dodati modele kroz svoju openlegion konfiguraciju.
...
da pohrani token u openlegion auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
OpenLegion koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate OpenLegion
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`openlegion.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Prilagodite putem `openlegion.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s openlegion:
...
Za korištenje Google Vertex AI s OpenLegion:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće openlegion.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem openlegiona, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [openlegion-helicone-session](https://github.com/H2Shami/openlegion-helicone-session) da automatski prijavite svaki OpenLegion razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki OpenLegion razgovor naveden kao zasebna sesija.
...
Možete konfigurirati openlegion za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati openlegion za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati openlegion za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za OpenLegion. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/openlegion) za detalje.
:::
...
Da biste koristili Ollama Cloud s OpenLegion:
...
3. Kopirajte API ključ za korištenje u OpenLegion.
...
6. **Važno**: Prije upotrebe modela oblaka u OpenLegion, morate lokalno povući informacije o modelu:
...
OpenLegion Zen je lista testiranih i verifikovanih modela koju je obezbedio OpenLegion tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>OpenLegion Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **OpenLegion Zen**.

```txt
   /connect
```

3. Unesite svoj OpenLegion API ključ.
...
Također možete dodati dodatne modele putem vaše openlegion konfiguracije.
...
5. Također ih možete prilagoditi putem vaše openlegion konfiguracije. Evo primjera navođenja provajdera
...
Također možete prilagoditi modele kroz svoju openlegion konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s openlegion-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `openlegion.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju OpenLegion da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `openlegion auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite OpenLegion konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj openlegion konfiguraciji.
