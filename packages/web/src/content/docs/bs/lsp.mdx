---
title: LSP serveri
description: OpenLegion se integriše sa vašim LSP serverima.
---

OpenLegion se može integrisati sa Language Server Protocol (LSP) serverima kako bi koristio dijagnostiku kao feedback za agenta.

## Ugrađeni

OpenLegion dolazi sa nekoliko ugrađenih LSP servera za popularne jezike:
...
:::note
Možete onemogućiti automatska preuzimanja LSP servera tako što ćete postaviti varijablu okruženja `OPENLEGION_DISABLE_LSP_DOWNLOAD` na `true`.
:::
...
Kada je LSP omogućen i openlegion otvori fajl, on:
...
Možete omogućiti i prilagoditi LSP servere kroz `lsp` odjeljak u vašoj openlegion konfiguraciji.
