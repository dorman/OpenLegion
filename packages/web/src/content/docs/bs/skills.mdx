...
Agent skills omogucavaju OpenLegion da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
OpenLegion pretrazuje ove lokacije:

- Konfiguracija projekta: `.openlegion/skills/<name>/SKILL.md`
- Globalna konfiguracija: `~/.config/openlegion/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, OpenLegion ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.openlegion/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.

Globalne definicije se takoder ucitavaju iz `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` i `~/.agents/skills/*/SKILL.md`.
...
Kreirajte `.openlegion/skills/git-release/SKILL.md` ovako:
...
OpenLegion navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `openlegion.json`:
...
**Za ugradene agente** (u `openlegion.json`):
