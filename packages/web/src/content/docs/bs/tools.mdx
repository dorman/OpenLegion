...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. OpenLegion dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u OpenLegion.
...
:::note
Ovaj alat je dostupan samo kada je `OPENLEGION_EXPERIMENTAL_LSP_TOOL=true` (ili `OPENLEGION_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz OpenLegion provajdera ili kada je varijabla `OPENLEGION_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju OpenLegion:
