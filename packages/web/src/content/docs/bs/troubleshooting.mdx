...
Da biste otklonili probleme s OpenLegion, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openlegion\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `openlegion --log-level DEBUG`.

---

## Pohrana

OpenLegion pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/openlegion/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\openlegion`
...
OpenLegion Desktop pokreće lokalni OpenLegion server (`openlegion-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `OpenLegion` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/openlegion/openlegion.jsonc` (ili `~/.config/openlegion/openlegion.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/openlegion/openlegion.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openlegion\openlegion.jsonc`
...
OpenLegion također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/openlegion/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\openlegion\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga OpenLegion mogao ponovo izgraditi.

1. Potpuno zatvorite OpenLegion Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/openlegion`
- **Linux**: obrišite `~/.cache/openlegion` (ili pokrenite `rm -rf ~/.cache/openlegion`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\openlegion`

3. Ponovo pokrenite OpenLegion Desktop.

---

### Rješavanje problema sa vezom na serveru

OpenLegion Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `openlegion.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `OPENLEGION_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `OPENLEGION_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, OpenLegion Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa OpenLegion karakteristikama.

---

### Obavještenja se ne prikazuju

OpenLegion Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za OpenLegion u postavkama vašeg OS-a, i
...
1. Zatvorite OpenLegion Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka OpenLegion Desktop aplikacije):

- `openlegion.settings.dat` (URL zadanog servera za desktop)
- `openlegion.global.dat` i `openlegion.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Ako imate problema s OpenLegion:
...
Da saznate kojim modelima imate pristup, pokrenite `openlegion models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\openlegion`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. OpenLegion dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\openlegion`

2. Ponovo pokrenite OpenLegion da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti OpenLegion da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
OpenLegion će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
