---
title: Konfiguracija
description: Korištenje OpenLegion JSON konfiguracije.
---

Možete konfigurirati OpenLegion koristeći JSON konfiguracijski fajl.

---

## Format

OpenLegion podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/openlegion`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/openlegion/openlegion.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENLEGION_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`openlegion.json` u projektu) - postavke specifične za projekat
5. **`.openlegion` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`OPENLEGION_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.openlegion` i `~/.config/openlegion` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/openlegion` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu OpenLegion konfiguraciju u `~/.config/openlegion/openlegion.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.

Za postavke specifične za TUI, koristite `~/.config/openlegion/tui.json`.
...
Dodajte `openlegion.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se OpenLegion pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `OPENLEGION_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENLEGION_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.openlegion` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.openlegion`, tako da **može nadjačati** njihove postavke.

---

## Šema

Konfiguracijski fajl ima šemu koja je definirana u [**`openlegion.dev/config.json`**](https://openlegion.dev/config.json).

TUI konfiguracija koristi [**`openlegion.dev/tui.json`**](https://openlegion.dev/tui.json).
...
Koristite `OPENLEGION_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `openlegion.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `openlegion serve` i `openlegion web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš OpenLegion server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `openlegion.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj OpenLegion konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, OpenLegion pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/openlegion/agents/` ili `.openlegion/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, OpenLegion će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`openlegion run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/openlegion/commands/` ili `.openlegion/commands/`. [Saznajte više ovdje](/docs/commands).
...
OpenLegion će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, OpenLegion **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju OpenLegion sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.openlegion/plugins/` ili `~/.config/openlegion/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite OpenLegion da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
