---
title: Server
description: Komunicirajte s openlegion serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `openlegion serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji openlegion klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `openlegion.local` |
...
Postavite `OPENLEGION_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `openlegion`, ili postavite `OPENLEGION_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `openlegion serve` i za `openlegion web`.
...
Kada pokrenete `openlegion`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite openlegion server za programsku interakciju sa openlegion-om.
:::

Ova arhitektura omogucava openlegion podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `openlegion serve` da startate standalone server. Ako je openlegion TUI vec pokrenut, `openlegion serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste OpenLegion [IDE](/docs/ide) pluginovi.
...
openlegion server izlaže sljedece API-je.
