...
U OpenLegion mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, OpenLegion koristi nasu `openlegion` temu.
...
OpenLegion dolazi sa vise ugradenih tema.
...
- Zele da OpenLegion odgovara izgledu njihovog terminala
...
OpenLegion podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
2. **Korisnicki config direktorij** - `~/.config/openlegion/themes/*.json` ili `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Korijenski direktorij projekta** - `<project-root>/.openlegion/themes/*.json`
4. **Trenutni radni direktorij** - `./.openlegion/themes/*.json`
