...
Možete dodati vanjske alate u OpenLegion koristeći _Model Context Protocol_, ili MCP. OpenLegion podržava i lokalne i udaljene servere.
...
Možete definirati MCP servere u vašoj [OpenLegion Config](https://openlegion.dev/docs/config/) pod `mcp`. Dodajte svaki MCP sa jedinstvenim imenom. Možete se pozvati na taj MCP po imenu kada tražite LLM.
...
Organizacije mogu obezbijediti zadane MCP servere preko svoje krajnje tačke `.well-known/openlegion`. Ovi serveri mogu biti onemogućeni prema zadanim postavkama, omogućavajući korisnicima da se odluče za one koji su im potrebni.
...
OpenLegion automatski rukuje OAuth autentifikacijom za udaljene MCP servere. Kada server zahtijeva autentifikaciju, OpenLegion će:
...
Ako server zahtijeva autentifikaciju, OpenLegion će vas tražiti prijavu pri prvom korištenju. Ako se to ne desi, možete [ručno pokrenuti tok](#authenticating) naredbom `openlegion mcp auth <server-name>`.
...
Komanda `mcp auth` otvara pretraživač za autorizaciju. Nakon odobrenja, OpenLegion sigurno čuva tokene u `~/.local/share/openlegion/mcp-auth.json`.
...
Vaši MCP serveri su dostupni kao alati u OpenLegion, zajedno s ugrađenim alatima. Možete njima upravljati kroz OpenLegion konfiguraciju kao i bilo kojim drugim alatom.
...
Ovo će otvoriti prozor pretraživača da završite OAuth tok i povežete OpenLegion sa vašim Sentry nalogom.
