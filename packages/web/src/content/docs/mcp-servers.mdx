...
You can add external tools to OpenLegion using the _Model Context Protocol_, or MCP. OpenLegion supports both local and remote servers.
...
You can define MCP servers in your [OpenLegion Config](https://openlegion.dev/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/openlegion` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
OpenLegion automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, OpenLegion will:
...
If the server requires authentication, OpenLegion will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `openlegion mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, OpenLegion will store the tokens securely in `~/.local/share/openlegion/mcp-auth.json`.
...
Your MCPs are available as tools in OpenLegion, alongside built-in tools. So you can manage them through the OpenLegion config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect OpenLegion to your Sentry account.
