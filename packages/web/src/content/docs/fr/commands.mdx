...
Créez `.openlegion/commands/test.md` :
...
Vous pouvez ajouter des commandes personnalisées via la configuration OpenLegion ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre OpenLegion [config](/docs/config) :
...
- Par projet : `.openlegion/commands/`
...
openlegion comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
