---
title: Plugins
description: Écrivez vos propres plugins pour étendre OpenLegion.
---

Les plugins vous permettent d'étendre OpenLegion en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de OpenLegion.
...
- `.openlegion/plugins/` - Plugins au niveau du projet
- `~/.config/openlegion/plugins/` - Plugins globaux
...
Les **plugins npm** sont installés automatiquement à l'aide de Bun au démarrage. Les packages et leurs dépendances sont mis en cache dans `~/.cache/openlegion/node_modules/`.
...
1. Configuration globale (`~/.config/openlegion/openlegion.json`)
2. Configuration du projet (`openlegion.json`)
3. Répertoire global des plugins (`~/.config/openlegion/plugins/`)
4. Répertoire des plugins du projet (`.openlegion/plugins/`)
...
OpenLegion exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK openlegion pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre openlegion.
...
:::note
Si vous utilisez l'application de bureau OpenLegion, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez openlegion de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à openlegion :
...
L'assistant `tool` crée un outil personnalisé que openlegion peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour openlegion aux côtés des outils intégrés.
...
Niveaux : `debug`, `info`, `warn`, `error`. Voir la [documentation du SDK](https://openlegion.dev/docs/sdk) pour plus de détails.
