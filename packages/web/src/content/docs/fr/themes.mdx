...
Avec OpenLegion, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, OpenLegion utilise notre propre thème `openlegion`.
...
OpenLegion est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que OpenLegion corresponde à l'apparence de leur terminal
...
OpenLegion prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
2. **Répertoire de configuration utilisateur** - Défini dans `~/.config/openlegion/themes/*.json` ou `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Répertoire racine du projet** - Défini dans `<project-root>/.openlegion/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.openlegion/themes/*.json`
...
```bash no-frame
mkdir -p ~/.config/openlegion/themes
vim ~/.config/openlegion/themes/my-theme.json
```

Et pour des thèmes spécifiques au projet.

```bash no-frame
mkdir -p .openlegion/themes
vim .openlegion/themes/my-theme.json
```
...
{
  "$schema": "https://openlegion.dev/theme.json",
  "defs": {
    "nord0": "#2E3440",
    "nord1": "#3B4252",
    "nord2": "#434C5E",
