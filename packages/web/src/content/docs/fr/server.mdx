---
title: Serveur
description: Interagissez avec le serveur openlegion via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

La commande `openlegion serve` exécute un serveur HTTP sans tête qui expose un point de terminaison OpenAPI qu'un client openlegion peut utiliser.
...
| `--mdns-domain` | Nom de domaine personnalisé pour le service mDNS   | `openlegion.local` |
...
Définissez `OPENLEGION_SERVER_PASSWORD` pour protéger le serveur avec l'authentification de base HTTP. Le nom d'utilisateur est par défaut `openlegion`, ou définissez `OPENLEGION_SERVER_USERNAME` pour le remplacer. Cela s’applique à la fois à `openlegion serve` et à `openlegion web`.
...
Lorsque vous exécutez `openlegion`, il démarre un TUI et un serveur. Où le TUI est le
...
:::tip
Utilisez le serveur openlegion pour interagir avec openlegion par programmation.
:::

Cette architecture permet à openlegion de prendre en charge plusieurs clients et vous permet d'interagir avec openlegion par programme.

Vous pouvez exécuter `openlegion serve` pour démarrer un serveur autonome. Si vous avez le
openlegion TUI en cours d'exécution, `openlegion serve` démarrera un nouveau serveur.
...
Le point de terminaison [`/tui`](#tui) peut être utilisé pour piloter le TUI via le serveur. Par exemple, vous pouvez pré-remplir ou exécuter une invite. Cette configuration est utilisée par les plugins OpenLegion [IDE](/docs/ide).
...
Le serveur openlegion expose les API suivantes.
