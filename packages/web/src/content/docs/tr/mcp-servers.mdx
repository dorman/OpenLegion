...
_Model Bağlam Protokolü_ veya MCP kullanarak openlegion'a harici araçlar ekleyebilirsiniz. openlegion hem yerel hem de uzak sunucuları destekler.
...
MCP sunucularını [openlegion Config](https://openlegion.dev/docs/config/)'nizde `mcp` altında tanımlayabilirsiniz. Her MCP'ü benzersiz bir adla ekleyin. LLM isteminde bulunurken bu MCP adına başvurabilirsiniz.
...
Kuruluşlar, `.well-known/openlegion` uç noktaları aracılığıyla varsayılan MCP sunucularını sağlayabilir. Bu sunucular varsayılan olarak devre dışı bırakılarak kullanıcıların ihtiyaç duydukları sunucuları seçmelerine olanak tanınabilir.
...
openlegion, uzak MCP sunucuları için OAuth kimlik doğrulamasını otomatik olarak yönetir. Bir sunucu kimlik doğrulaması gerektirdiğinde openlegion şunları yapar:
...
Sunucu kimlik doğrulama gerektiriyorsa openlegion, onu ilk kez kullanmaya çalıştığınızda kimlik doğrulamanızı isteyecektir. Değilse, `openlegion mcp auth <server-name>` ile [manually trigger the flow](#authenticating) yapabilirsiniz.
...
`mcp auth` komutu tarayıcınızı yetkilendirme için açacaktır. Yetkilendirmenizin ardından openlegion, belirteçleri `~/.local/share/openlegion/mcp-auth.json`'de güvenli bir şekilde saklar.
...
MCP'leriniz yerleşik araçların yanı sıra openlegion'da araç olarak mevcuttur. Böylece bunları diğer araçlar gibi openlegion yapılandırması aracılığıyla yönetebilirsiniz.
...
Bu, OAuth akışını tamamlamak ve openlegion'u Sentry hesabınıza bağlamak için bir tarayıcı penceresi açacaktır.
