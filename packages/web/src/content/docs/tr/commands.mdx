...
`.openlegion/commands/test.md` oluştur:
...
openlegion işlemleri aracılığıyla veya `commands/` dizininde işaretleme dosyalarının oluşturulması özel komutlar ile yapılabilir.

---

### JSON

openlegion [config](/docs/config)'deki `command` seçeneğini kullanın:
...
- Küresel: `~/.config/openlegion/commands/`
- Proje başına: `.openlegion/commands/`
...
openlegion, `/init`, `/undo`, `/redo`, `/share`, `/help` gibi çeşitli yerleşik komutlar içerir; [learn more](/docs/tui#commands).
