...
openlegion ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak openlegion kendi `openlegion` temasını kullanır.
...
openlegion birden fazla yerleşik temayla gelir.
...
- openlegion'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
openlegion, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
2. **Kullanıcı yapılandırma dizini** - `~/.config/openlegion/themes/*.json` veya `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Proje kök dizini** - `<project-root>/.openlegion/themes/*.json`
4. **Geçerli çalışma dizini** - `./.openlegion/themes/*.json`
