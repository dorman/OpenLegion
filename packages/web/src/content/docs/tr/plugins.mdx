---
title: Eklentiler
description: OpenLegion'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek OpenLegion'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya OpenLegion'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.openlegion/plugins/` - Proje düzeyinde eklentiler
- `~/.config/openlegion/plugins/` - Genel eklentiler
...
**npm eklentileri** başlangıçta Bun kullanılarak otomatik olarak yüklenir. Paketler ve bağımlılıkları `~/.cache/openlegion/node_modules/`'da önbelleğe alınır.
...
1. Global config (`~/.config/openlegion/openlegion.json`)
2. Project config (`openlegion.json`)
3. Global eklenti dizini (`~/.config/openlegion/plugins/`)
4. Proje eklenti dizini (`.openlegion/plugins/`)
...
OpenLegion bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir OpenLegion SDK istemcisi.
...
OpenLegion'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
OpenLegion masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
OpenLegion'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca OpenLegion'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, OpenLegion'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra OpenLegion'a da hazır olacaktır.
...
Seviyeler: `debug`, `info`, `warn`, `error`. Ayrıntılar için [SDK belgelerine](https://openlegion.dev/docs/sdk) bakın.
