...
OpenLegion Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, OpenLegion'daki diğer sağlayıcılar gibi çalışır. OpenLegion Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve OpenLegion'u kullanmak için bunu kullanmanıza gerek yoktur.
...
OpenLegion Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

OpenLegion Go, OpenLegion'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>OpenLegion Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `OpenLegion Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye OpenLegion Go'ya abone olabilir.
:::
...
OpenLegion Go aşağıdaki limitleri içerir:
...
OpenLegion yapılandırmanızdaki [model id](/docs/config/#models) formatı `openlegion-go/<model-id>` şeklindedir. Örneğin, Kimi K2.6 için yapılandırmanızda `openlegion-go/kimi-k2.6` kullanmalısınız.
...
OpenLegion Go'yu şu amaçlarla oluşturduk:
...
4. OpenLegion ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
