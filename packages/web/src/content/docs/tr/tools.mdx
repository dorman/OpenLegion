...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. openlegion yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
openlegion içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `OPENLEGION_EXPERIMENTAL_LSP_TOOL=true` (veya `OPENLEGION_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca openlegion provider kullanılırken veya `OPENLEGION_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

openlegion başlatırken etkinleştirmek için:
