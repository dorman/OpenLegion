...
Ajan becerileri, openlegion'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
openlegion su konumlari tarar:

- Proje konfigurasyonu: `.openlegion/skills/<name>/SKILL.md`
- Genel konfigurasyon: `~/.config/openlegion/skills/<name>/SKILL.md`
...
Proje yerel yollarinda openlegion, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.openlegion/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.

Genel tanimlar da `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` ve `~/.agents/skills/*/SKILL.md` yollarindan yuklenir.
...
`.openlegion/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
openlegion, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`openlegion.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`openlegion.json` icinde):
