---
title: Sağlayıcılar
description: openlegion'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

openlegion, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı openlegion yapılandırmanızda yapılandırın.
...
`~/.local/share/openlegion/auth.json` içinde.

---

### Yapılandırma

Sağlayıcıları openlegion'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
OpenLegion Zen, openlegion ekibi tarafından sağlanan modellerin bir listesidir.
openlegion ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz OpenLegion Zen ile başlamanızı öneririz.
:::
...
openlegion'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## OpenLegion Go

OpenLegion Go, openlegion ile iyi çalıştığı test edilmiş ve doğrulanmış, openlegion ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `OpenLegion Go`'yu seçin ve [openlegion.dev/auth](https://openlegion.dev/zen) adresine gidin.
...
Amazon Bedrock'u openlegion ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   openlegion'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `openlegion.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi openlegion'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
openlegion'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   openlegion'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
:::note[uyumluluk notu]
openlegion, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. openlegion'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`openlegion.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`openlegion.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi openlegion'la kullanmak için:
...
Google Vertex AI'yi openlegion ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları openlegion çalıştırılırken ayarlayın.
...
Helicone'un openlegion aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her openlegion görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [openlegion-helicone-session](https://github.com/H2Shami/openlegion-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her openlegion görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
openlegion'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla openlegion'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla openlegion'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini openlegion için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/openlegion)'a bakın.
:::
...
Ollama Cloud'u openlegion ile kullanmak için:
...
3. openlegion'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: openlegion'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
OpenLegion Zen, openlegion ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>OpenLegion Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **OpenLegion Zen**'i arayın.

   ```txt
   /connect
   ```

3. openlegion API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   openlegion yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca openlegion yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
Modelleri openlegion yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı openlegion ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `openlegion.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları openlegion'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `openlegion auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için openlegion yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, openlegion yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
