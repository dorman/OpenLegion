...
openlegion'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
openlegion, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
OpenLegion, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

OpenLegion iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `openlegion.json` yapılandırma dosyanızda yapılandırın:
...
- Proje başına: `.openlegion/agents/`
...
Sıcaklık belirtilmezse openlegion modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global openlegion yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
openlegion yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [OpenLegion Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `openlegion/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `openlegion models` komutunu çalıştırın.
:::
...
:::tip
Paylaşmak istediğiniz bir agent'ınız var mı? [Submit a PR](https://github.com/dorman/OpenLegion).
:::
