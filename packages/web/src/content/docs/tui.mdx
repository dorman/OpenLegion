---
title: TUI
description: Using the OpenLegion terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

OpenLegion provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running OpenLegion starts the TUI for the current directory.
...
When using the OpenLegion TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to OpenLegion. Allows you to select from available providers and add their API keys.
...
Exit OpenLegion. _Aliases_: `/quit`, `/q`
...
This is separate from `openlegion.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long OpenLegion waits after the leader key. Defaults to `2000`.
...
Use `OPENLEGION_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `openlegion.default`.
