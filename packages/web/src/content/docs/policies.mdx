---
title: Policies
description: Control which configured resources OpenLegion may use.
---

Policies control whether OpenLegion may perform an action on a named resource. This feature is experimental and is configured with the `experimental.policies` array in `openlegion.json`.

Policies are separate from [permissions](/docs/permissions). Permissions control what tools can do during a session, while policies control whether OpenLegion may use a resource such as an LLM provider.
...
OpenLegion currently supports one policy action:
