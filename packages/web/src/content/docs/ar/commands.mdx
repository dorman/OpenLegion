...
أنشئ الملف `.openlegion/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات OpenLegion أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ OpenLegion:
...
- على مستوى النظام: `~/.config/openlegion/commands/`
- لكل مشروع: `.openlegion/commands/`
...
يتضمن openlegion عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
