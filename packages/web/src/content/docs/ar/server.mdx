---
title: الخادم
description: تفاعل مع خادم openlegion عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `openlegion serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل openlegion استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `openlegion.local` |
...
عيّن `OPENLEGION_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `openlegion`، أو عيّن `OPENLEGION_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `openlegion serve` و `openlegion web`.
...
عند تشغيل `openlegion` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم openlegion للتفاعل مع openlegion برمجيا.
:::

تتيح هذه البنية لـ openlegion دعم عدة عملاء وتمكّنك من التفاعل مع openlegion برمجيا.

يمكنك تشغيل `openlegion serve` لبدء خادم مستقل. إذا كانت واجهة openlegion في terminal (TUI)
قيد التشغيل، فسيبدأ `openlegion serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات OpenLegion لـ [IDE](/docs/ide).
...
يعرض خادم openlegion واجهات API التالية.
