...
تتيح مهارات الوكيل لـ OpenLegion اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث OpenLegion في هذه المواقع:

- إعدادات المشروع: `.openlegion/skills/<name>/SKILL.md`
- إعدادات عامة: `~/.config/openlegion/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج OpenLegion صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.openlegion/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.

كما تُحمَّل التعريفات العامة أيضاً من `~/.config/openlegion/skills/*/SKILL.md` و `~/.claude/skills/*/SKILL.md` و `~/.agents/skills/*/SKILL.md`.
...
أنشئ `.openlegion/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض OpenLegion المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `openlegion.json`:
...
**للوكلاء المدمجين** (في `openlegion.json`):
