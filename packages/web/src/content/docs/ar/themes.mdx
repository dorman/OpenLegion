...
مع OpenLegion يمكنك الاختيار من بين عدة سمات مدمجة، أو استخدام سمة تتكيّف مع سمة terminal لديك، أو تعريف سمة مخصصة خاصة بك.

افتراضيًا، يستخدم OpenLegion سمتنا `openlegion`.
...
يأتي OpenLegion مع عدة سمات مدمجة.
...
- يريدون أن يطابق OpenLegion مظهر terminal لديهم
...
يدعم OpenLegion نظام سمات مرنًا قائمًا على JSON يتيح للمستخدمين إنشاء السمات وتخصيصها بسهولة.
...
2. **مجلد ضبط المستخدم** - معرّف في `~/.config/openlegion/themes/*.json` أو `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **مجلد جذر المشروع** - معرّف في `<project-root>/.openlegion/themes/*.json`
4. **مجلد العمل الحالي** - معرّف في `./.openlegion/themes/*.json`
