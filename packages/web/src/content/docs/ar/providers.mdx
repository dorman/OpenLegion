---
title: المزوّدون
description: استخدام أي موفّر LLM في OpenLegion.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم OpenLegion ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات OpenLegion.
...
في `~/.local/share/openlegion/auth.json`.

---

### الإعدادات

يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات OpenLegion.
...
OpenLegion Zen هي قائمة نماذج يوفّرها فريق OpenLegion وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenLegion. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع OpenLegion Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر openlegion، ثم انتقل إلى [openlegion.dev/auth](https://openlegion.dev/auth).
...
يعمل مثل أي مزوّد آخر في OpenLegion واستخدامه اختياري بالكامل.

---

## OpenLegion Go

OpenLegion Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق OpenLegion، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع OpenLegion.

1. شغّل الأمر `/connect` في TUI، واختر `OpenLegion Go`، ثم انتقل إلى [openlegion.dev/auth](https://openlegion.dev/zen).
...
لاستخدام Amazon Bedrock مع OpenLegion:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل openlegion:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `openlegion.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل OpenLegion غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين openlegion لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل openlegion بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات openlegion.
...
تخزين الرمز في مساحة تخزين مصادقة openlegion.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم OpenLegion نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر OpenLegion
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`openlegion.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
لاستخدام اشتراك GitHub Copilot مع openlegion:
...
لاستخدام Google Vertex AI مع OpenLegion:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر openlegion، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [openlegion-helicone-session](https://github.com/H2Shami/openlegion-helicone-session) لتسجيل كل محادثة OpenLegion تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة OpenLegion مدرجة كجلسة مستقلة.
...
يمكنك ضبط openlegion لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط openlegion لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط openlegion لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع OpenLegion. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/openlegion) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع OpenLegion:
...
3. انسخ مفتاح API لاستخدامه في OpenLegion.
...
6. **مهم**: قبل استخدام نماذج السحابة في OpenLegion، يجب سحب معلومات النموذج محليا:
...
OpenLegion Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق OpenLegion. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>OpenLegion Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **OpenLegion Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ OpenLegion.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات openlegion.
...
5. يمكنك أيضا تخصيصها عبر إعدادات openlegion. إليك مثالا على تحديد مزوّد
...
يمكنك أيضا تخصيص النماذج عبر إعدادات openlegion. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع openlegion. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `openlegion.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ OpenLegion بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `openlegion auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات openlegion وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات openlegion.
