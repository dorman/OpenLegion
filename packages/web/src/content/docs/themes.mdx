...
With OpenLegion you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, OpenLegion uses our own `openlegion` theme.
...
OpenLegion comes with several built-in themes.
...
- Want OpenLegion to match their terminal's appearance
...
OpenLegion supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
2. **User config directory** - Defined in `~/.config/openlegion/themes/*.json` or `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Project root directory** - Defined in the `<project-root>/.openlegion/themes/*.json`
4. **Current working directory** - Defined in `./.openlegion/themes/*.json`
