---
title: LSP Servers
description: OpenLegion integrates with your LSP servers.
---

OpenLegion can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

OpenLegion comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `OPENLEGION_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and openlegion opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your openlegion config.
...
```json title="openlegion.json" {5-8}
{
  "$schema": "https://openlegion.dev/config.json",
  "lsp": {
    "rust": {
...
```json title="openlegion.json" {5-13}
{
  "$schema": "https://openlegion.dev/config.json",
  "lsp": {
    "custom-lsp": {
