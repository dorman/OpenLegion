...
Create `.openlegion/commands/test.md`:
...
You can add custom commands through the OpenLegion config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your OpenLegion [config](/docs/config):
...
- Per-project: `.openlegion/commands/`
...
openlegion includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
