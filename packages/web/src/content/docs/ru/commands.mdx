...
Создайте `.openlegion/commands/test.md`:
...
Вы можете добавлять собственные команды через конфигурацию openlegion или создав файлы Markdown в каталоге `commands/`.

---

### JSON

Используйте опцию `command` в вашем openlegion [config](/docs/config):
...
- Глобальный: `~/.config/openlegion/commands/`
- Для каждого проекта: `.openlegion/commands/`
...
openlegion включает несколько встроенных команд, таких как `/init`, `/undo`, `/redo`, `/share`, `/help`; [подробнее](/docs/tui#commands).
