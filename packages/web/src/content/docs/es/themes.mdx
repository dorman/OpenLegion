...
Con OpenLegion puede seleccionar uno de varios temas integrados, usar un tema que se adapte al tema de su terminal o definir su propio tema personalizado.

De forma predeterminada, OpenLegion usa nuestro propio tema `openlegion`.
...
OpenLegion viene con varios temas integrados.
...
- Quiere que OpenLegion coincida con la apariencia de su terminal
...
OpenLegion admite un sistema de temas flexible basado en JSON que permite a los usuarios crear y personalizar temas fácilmente.
...
2. **Directorio de configuración de usuario**: definido en `~/.config/openlegion/themes/*.json` o `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Directorio raíz del proyecto**: definido en `<project-root>/.openlegion/themes/*.json`
4. **Directorio de trabajo actual** - Definido en `./.openlegion/themes/*.json`
