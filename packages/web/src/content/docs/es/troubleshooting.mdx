...
Para depurar problemas con OpenLegion, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openlegion\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `openlegion --log-level DEBUG`.

---

## Almacenamiento

openlegion almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/openlegion/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\openlegion`
...
OpenLegion Desktop ejecuta un servidor OpenLegion local (el sidecar `openlegion-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `OpenLegion` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/openlegion/openlegion.jsonc` (o `~/.config/openlegion/openlegion.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/openlegion/openlegion.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openlegion\openlegion.jsonc`
...
OpenLegion también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/openlegion/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\openlegion\plugins`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que OpenLegion pueda reconstruirlo.

1. Salga de OpenLegion Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/openlegion`
- **Linux**: eliminar `~/.cache/openlegion` (o ejecutar `rm -rf ~/.cache/openlegion`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\openlegion`

3. Reinicie el escritorio OpenLegion.

---

### Solucionar problemas de conexión del servidor

OpenLegion Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `openlegion.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `OPENLEGION_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `OPENLEGION_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio OpenLegion requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de OpenLegion.

---

### Notificaciones que no se muestran

OpenLegion Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para OpenLegion en la configuración de su sistema operativo, y
...
1. Salga del escritorio OpenLegion.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio OpenLegion):

- `openlegion.settings.dat` (URL del servidor predeterminado de escritorio)
- `openlegion.global.dat` y `openlegion.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
Si tiene problemas con OpenLegion:
...
1. **Informar problemas el GitHub**
   ...
   [**github.com/dorman/OpenLegion/issues**](https://github.com/dorman/OpenLegion/issues)
...
2. **Únete a nuestro Discord**
   ...
   [**openlegion.dev/discord**](https://openlegion.dev/discord)
...
3. Asegúrese de tener la última versión con `openlegion upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `openlegion models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/openlegion
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\openlegion`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. openlegion instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\openlegion`

2. Reinicie openlegion para reinstalar los paquetes del proveedor más recientes.

Esto obligará a openlegion a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
openlegion detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
