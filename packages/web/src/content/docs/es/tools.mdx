...
Las herramientas permiten que LLM realice acciones en su código base. OpenLegion viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en OpenLegion.
...
:::note
Esta herramienta solo está disponible cuando `OPENLEGION_EXPERIMENTAL_LSP_TOOL=true` (o `OPENLEGION_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor OpenLegion o cuando la variable de entorno `OPENLEGION_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar OpenLegion:

```bash
OPENLEGION_ENABLE_EXA=1 openlegion
```
