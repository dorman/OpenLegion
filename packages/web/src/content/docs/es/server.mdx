---
title: Servidor
description: Interactuar con el servidor openlegion a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `openlegion serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente openlegion puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `openlegion.local` |
...
Configure `OPENLEGION_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `openlegion`, o configure `OPENLEGION_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `openlegion serve` como a `openlegion web`.
...
Cuando ejecuta `openlegion`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor openlegion para interactuar con openlegion mediante programación.
:::

Esta arquitectura permite que openlegion admita múltiples clientes y le permite interactuar con openlegion mediante programación.

Puede ejecutar `openlegion serve` para iniciar un servidor independiente. Si tienes el
openlegion TUI ejecutándose, `openlegion serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos OpenLegion [IDE](/docs/ide).
...
El servidor openlegion expone las siguientes API.
