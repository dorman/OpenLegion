---
title: Complementos
description: Escriba sus propios complementos para extender OpenLegion.
---

Los complementos le permiten extender OpenLegion al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de OpenLegion.
...
- `.openlegion/plugins/` - Complementos a nivel de proyecto
- `~/.config/openlegion/plugins/` - Complementos globales
...
```json title="openlegion.json"
{
  ...
  "plugin": ["openlegion-helicone-session", "openlegion-wakatime", "@my-org/custom-plugin"]
}
```
...
Los **complementos npm** se instalan automáticamente usando Bun al inicio. Los paquetes y sus dependencias se almacenan en caché en `~/.cache/openlegion/node_modules/`.
...
1. Configuración global (`~/.config/openlegion/openlegion.json`)
2. Configuración del proyecto (`openlegion.json`)
3. Directorio global de complementos (`~/.config/openlegion/plugins/`)
4. Directorio de complementos del proyecto (`.openlegion/plugins/`)
...
```json title=".openlegion/package.json"
{
  "dependencies": {
    "shescape": "^2.1.0"
  }
...
OpenLegion ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.

```ts title=".openlegion/plugins/my-plugin.ts"
...
```js title=".openlegion/plugins/example.js"
...
- `client`: Un cliente SDK openlegion para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar openlegion.
...
```js title=".openlegion/plugins/notification.js"
export const NotificationPlugin = async ({ project, client, $, directory, worktree }) => {
  return {
    event: async ({ event }) => {
      // Send notification on session completion
      if (event.type === "session.idle") {
        await $`osascript -e 'display notification "Session completed!" with title "openlegion"'`
      }
    },
  }
}
...
:::note
Si está utilizando la aplicación de escritorio OpenLegion, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que openlegion lea archivos `.env`:

```javascript title=".openlegion/plugins/env-protection.js"
...
```javascript title=".openlegion/plugins/inject-env.js"
...
Los complementos también pueden agregar herramientas personalizadas a openlegion:

```ts title=".openlegion/plugins/custom-tools.ts"
import { type Plugin, tool } from "@openlegion-ai/plugin"
...
El ayudante `tool` crea una herramienta personalizada a la que openlegion puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para openlegion junto con las herramientas integradas.
...
Niveles: `debug`, `info`, `warn`, `error`. Consulte la [documentación del SDK](https://openlegion.dev/docs/sdk) para obtener más detalles.
...
```ts title=".openlegion/plugins/compaction.ts"
import type { Plugin } from "@openlegion-ai/plugin"
...
```ts title=".openlegion/plugins/custom-compaction.ts"
import type { Plugin } from "@openlegion-ai/plugin"
