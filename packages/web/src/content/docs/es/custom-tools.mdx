...
description: Cree herramientas que LLM pueda llamar en openlegion.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de openlegion como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.openlegion/tools/` de tu proyecto.
- O de forma global, colocándolos en `~/.config/openlegion/tools/`.
...
```ts title=".openlegion/tools/database.ts" {1}
...
```ts title=".openlegion/tools/math.ts"
import { tool } from "@openlegion-ai/plugin"
...
```ts title=".openlegion/tools/bash.ts"
import { tool } from "@openlegion-ai/plugin"
...
```ts title=".openlegion/tools/project.ts" {8}
import { tool } from "@openlegion-ai/plugin"
...
```python title=".openlegion/tools/add.py"
import sys
...
```ts title=".openlegion/tools/python-add.ts" {10}
import { tool } from "@openlegion-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".openlegion/tools/add.py")
