...
Las habilidades del agente permiten a OpenLegion descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
OpenLegion busca estas ubicaciones:

- Configuración del proyecto: `.openlegion/skills/<name>/SKILL.md`
- Configuración global: `~/.config/openlegion/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, OpenLegion sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.openlegion/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.

Las definiciones globales también se cargan desde `~/.config/openlegion/skills/*/SKILL.md`, `~/.claude/skills/*/SKILL.md` y `~/.agents/skills/*/SKILL.md`.
...
Crea `.openlegion/skills/git-release/SKILL.md` así:
...
OpenLegion enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `openlegion.json`:
...
**Para agentes integrados** (en `openlegion.json`):
