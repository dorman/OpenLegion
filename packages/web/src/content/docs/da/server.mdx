---
title: Server
description: Interager med openlegion-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `openlegion serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en openlegion-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `openlegion.local` |
...
Indstil `OPENLEGION_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `openlegion`, eller indstil `OPENLEGION_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `openlegion serve` og `openlegion web`.
...
Når du kører `openlegion` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug openlegion-serveren til at interagere med openlegion programmatisk.
:::

Denne arkitektur lader openlegion understøtte flere klienter og lader dig interagere med openlegion programmatisk.

Du kan køre `openlegion serve` for at starte en selvstændig server. Hvis du har
openlegion TUI kørende, vil `openlegion serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af OpenLegion [IDE](/docs/ide) plugins.
...
OpenLegion-serveren viser følgende API'er.
