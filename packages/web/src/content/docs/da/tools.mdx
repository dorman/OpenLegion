...
Her er alle de innebygde verktøyene som er tilgængelige i OpenLegion.
...
:::note
Dette verktøyet er bare tilgængelig når `OPENLEGION_EXPERIMENTAL_LSP_TOOL=true` (eller `OPENLEGION_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger OpenLegion-leverandøren eller når miljøvariabelen `OPENLEGION_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
