---
title: Plugins
description: Skriv dine egne plugins for at udvide OpenLegion.
---

Plugins giver dig mulighed for at udvide OpenLegion ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre OpenLegions standardadfærd.
...
- `.openlegion/plugins/` - Plugins på projektniveau
...
**npm plugins** installeres automatisk ved hjælp af Bun ved opstart. Pakker og deres afhængigheder cachelagres i `~/.cache/openlegion/node_modules/`.
...
1. Global konfiguration (`~/.config/openlegion/openlegion.json`)
2. Projektkonfiguration (`openlegion.json`)
3. Global plugin-mappe (`~/.config/openlegion/plugins/`)
4. Projekt plugin bibliotek (`.openlegion/plugins/`)
...
OpenLegion kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En openlegion SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide openlegion.
...
:::note
Hvis du bruger OpenLegion desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå openlegion i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til openlegion:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som openlegion kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for openlegion sammen med indbyggede værktøjer.
...
Niveauer: `debug`, `info`, `warn`, `error`. Se [SDK documentation](https://openlegion.dev/docs/sdk) for detaljer.
