...
Med OpenLegion kan du velge fra ett av flere innebygde temaer, bruge et tema som tilpasser seg terminaltemaet ditt, eller definere ditt eget tilpassede tema.

Som standard bruger OpenLegion vårt eget `openlegion`-tema.
...
OpenLegion kommer med flere innebygde temaer.
...
- Vil at OpenLegion skal matche terminalens utseende
...
OpenLegion støtter et fleksibelt JSON-basert temasystem som lar brugere enkelt lage og tilpasse temaer.
...
2. **Brugerkonfigurasjonskatalog** - Definert i `~/.config/openlegion/themes/*.json` eller `$XDG_CONFIG_HOME/openlegion/themes/*.json`
3. **Prosjektrotkatalog** - Definert i `<project-root>/.openlegion/themes/*.json`
4. **Nuværende arbejdskatalog** - Definert i `./.openlegion/themes/*.json`
