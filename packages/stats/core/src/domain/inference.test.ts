...
describe("inference stat normalization", () => {
  ...
  test("uses provider.model to resolve openlegion route providers", () => {
    ...
    expect(statProvider("big-pickle", "claude-sonnet-4-5", "openlegion")).toBe("anthropic")
    expect(statProvider("big-pickle", "gpt-5", "openlegion")).toBe("openai")
    expect(statProvider("big-pickle", "", "openlegion")).toBe("unknown")
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate({ ...aggregate("big-pickle", "openlegion"), provider_model: "claude-sonnet-4-5" }),
    ).toMatchObject([
      {
        provider: "anthropic",
  ...
  test("provider aggregates never keep openlegion as the provider", () => {
    expect(toProviderAggregate({ ...aggregate("big-pickle", "openlegion"), provider_model: "gpt-5" })).toMatchObject([
      { provider: "openai" },
    ])
    expect(toProviderAggregate(aggregate("big-pickle", "openlegion"))).toMatchObject([{ provider: "unknown" }])
  })

  test("geo aggregates never keep openlegion or big-pickle dimensions", () => {
    expect(toGeoAggregate({ ...aggregate("big-pickle", "openlegion"), country: "US" })).toMatchObject([
