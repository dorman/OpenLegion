...
Schema-first LLM core for openlegion. One typed request, response, event, and tool language; provider quirks live in adapters, not in calling code.

```ts
import { Effect } from "effect"
import { LLM, LLMClient } from "@openlegion-ai/llm"
...
```ts
import { OpenAI, CloudflareAIGateway } from "@openlegion-ai/llm/providers"
