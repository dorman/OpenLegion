...
import { type Session } from "@openlegion-ai/sdk/v2/client"
import {
  childSessionOnPath,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("openlegion://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("openlegion://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("openlegion://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("openlegion://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("openlegion://open-project")).toBeUndefined()
    expect(parseDeepLink("openlegion://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "openlegion://open-project?directory=/a",
      "openlegion://other?directory=/b",
      "openlegion://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("openlegion://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("openlegion://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("openlegion://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("openlegion://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "openlegion://new-session?directory=/a",
      "openlegion://open-project?directory=/b",
      "openlegion://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __OPENLEGION__: {
        deepLinks: ["openlegion://open-project?directory=/a"],
      },
    } as unknown as Window & { __OPENLEGION__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["openlegion://open-project?directory=/a"])
