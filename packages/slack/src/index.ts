...
import { createOpencode, type ToolPart } from "@openlegion-ai/sdk"
...
console.log("🚀 Starting openlegion server...")
const openlegion = await createOpencode({
  port: 0,
})
...
void (async () => {
  const events = await openlegion.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new openlegion session...")
    const { client, server } = openlegion
    ...
    console.log("✅ Created openlegion session:", createResult.data.id)
  ...
  console.log("📝 Sending to openlegion:", message.text)
