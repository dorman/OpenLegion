...
export const layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const hasPendingPrompt = Effect.fn("SessionRunner.hasPendingPrompt")(function* (sessionID: SessionSchema.ID) {
        const state = yield* store.turnState(sessionID)
        return state.unsettled || state.latestPromptCursor !== undefined && state.latestPromptCursor > (state.latestStartedPromptCursor ?? -1)
      })
      ...
      const runTurn = Effect.fn("SessionRunner.runTurn")(function* (session: SessionSchema.Info) {
        ...
        // Read the cursor before context. A racing prompt may cause one harmless extra
        // turn, but no prompt can be marked consumed before it was included in context.
        const turnState = yield* store.turnState(session.id)
        ...
        const turn = yield* events.publish(SessionEvent.Turn.Started, {
          ...
          ...(turnState.latestPromptCursor === undefined ? {} : { promptCursor: turnState.latestPromptCursor }),
        })
        ...
        const settled = yield* Effect.raceFirst(FiberSet.join(settlements), FiberSet.awaitEmpty(settlements)).pipe(Effect.exit)
        const attempt = stream._tag === "Failure" ? stream : settled
        yield* events.publish(SessionEvent.Turn.Settled, {
          sessionID: session.id,
          timestamp: yield* DateTime.now,
          turnID: turn.id,
          outcome: attempt._tag === "Success" ? "completed" : Cause.hasInterruptsOnly(attempt.cause) ? "interrupted" : "failed",
        })
        if (attempt._tag === "Failure") return yield* Effect.failCause(attempt.cause)
        return needsContinuation
      }, Effect.scoped)

      const run = Effect.fn("SessionRunner.run")(function* (input: { readonly sessionID: SessionSchema.ID; readonly force?: boolean }) {
        const session = yield* getSession(input.sessionID)
        let needsContinuation = input.force === true
        for (let step = 0; step < MAX_STEPS; step++) {
          if (!needsContinuation && !(yield* hasPendingPrompt(input.sessionID))) return
          needsContinuation = yield* runTurn(session)
        }
        if (!needsContinuation && !(yield* hasPendingPrompt(input.sessionID))) return
