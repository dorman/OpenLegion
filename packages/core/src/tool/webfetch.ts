...
Use a more targeted tool when one is available. The URL is fetched as provided, including HTTP URLs and local or private network addresses. This tool is read-only. Large text results are truncated with an opaque managed resource URI for paging.`
          ...
          Effect.gen(function* () {
            ...
            const { body, contentType } = yield* Effect.gen(function* () {
              const response = yield* http
                .execute(request(parameters.url, parameters.format))
                .pipe(
                  Effect.catchIf(isCloudflareChallenge, () =>
                    http.execute(request(parameters.url, parameters.format, "opencode")),
                  ),
                )
