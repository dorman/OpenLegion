  ...
  test("applies tracked migrations to an empty database", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(yield* db.get(sql`SELECT count(*) as count FROM migration`)).toEqual({ count: 27 })
        expect(
          yield* db.all(
            sql`SELECT name FROM sqlite_master WHERE type = 'index' AND name IN ('event_aggregate_seq_idx', 'session_input_session_pending_seq_idx', 'session_message_session_idx', 'session_message_session_type_idx', 'session_message_session_seq_idx', 'session_message_session_type_seq_idx') ORDER BY name`,
          ),
        ).toEqual([
          { name: "event_aggregate_seq_idx" },
          { name: "session_input_session_pending_seq_idx" },
