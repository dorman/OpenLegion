...
import { ConfigReference } from "@openlegion-ai/core/config/reference"
...
import { Repository } from "@openlegion-ai/core/repository"
import { RepositoryCache } from "@openlegion-ai/core/repository-cache"
...
describe("ProjectReference", () => {
  it.live("uses the broad experimental flag unless references are explicitly configured", () =>
    withEnv(
      { OPENLEGION_EXPERIMENTAL: "true", OPENLEGION_EXPERIMENTAL_REFERENCES: undefined },
      Effect.sync(() => {
        expect(Flag.OPENLEGION_EXPERIMENTAL_REFERENCES).toBe(true)
      }),
    ).pipe(
      Effect.flatMap(() =>
        withEnv(
          { OPENLEGION_EXPERIMENTAL: "true", OPENLEGION_EXPERIMENTAL_REFERENCES: "false" },
          Effect.sync(() => {
            expect(Flag.OPENLEGION_EXPERIMENTAL_REFERENCES).toBe(false)
          }),
        ),
      ),
    ),
...
function withReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ OPENLEGION_EXPERIMENTAL_REFERENCES: "true" }, body)
}

function withoutReferences<A, E, R>(body: Effect.Effect<A, E, R>) {
  return withEnv({ OPENLEGION_EXPERIMENTAL: undefined, OPENLEGION_EXPERIMENTAL_REFERENCES: undefined }, body)
}
