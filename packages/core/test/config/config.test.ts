...
import { Config } from "@openlegion-ai/core/config"
import { ConfigProvider } from "@openlegion-ai/core/config/provider"
import { ConfigMigrateV1 } from "@openlegion-ai/core/v1/config/migrate"
...
import { Policy } from "@openlegion-ai/core/policy"
...
describe("Config", () => {
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(
                path.join(tmp.path, "openlegion.json"),
                JSON.stringify({ $schema: "middle", providers: { middle: provider } }),
              ),
              fs.writeFile(
                path.join(tmp.path, "openlegion.jsonc"),
                `{
          ...
          return yield* Effect.gen(function* () {
            ...
            yield* Effect.promise(() =>
              fs.writeFile(path.join(tmp.path, "openlegion.jsonc"), JSON.stringify({ $schema: "changed" })),
            )
            expect(
        ...
        Effect.gen(function* () {
          yield* Effect.promise(() =>
            Promise.all([
              ...
              fs.writeFile(path.join(tmp.path, "openlegion.json"), "{ invalid"),
              fs.writeFile(path.join(tmp.path, "openlegion.jsonc"), JSON.stringify({ providers: { invalid: true } })),
            ]),
          )
          return yield* Effect.gen(function* () {
            const config = yield* Config.Service
      ...
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.writeFile(
              path.join(global, "openlegion.json"),
              JSON.stringify({
                experimental: { policies: [{ effect: "deny", action: "provider.use", resource: "openai" }] },
              }),
            )
  ...
  it.live("loads global, ancestor, and .openlegion configuration up to the project boundary", () =>
    Effect.acquireRelease(
      Effect.promise(() => tmpdir()),
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
    ).pipe(
      Effect.flatMap((tmp) => {
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await fs.mkdir(path.join(root, ".openlegion"), { recursive: true })
            await fs.mkdir(path.join(directory, ".openlegion"), { recursive: true })
            await Promise.all([
              fs.writeFile(path.join(tmp.path, "openlegion.json"), JSON.stringify({ $schema: "outside" })),
              fs.writeFile(path.join(global, "openlegion.json"), JSON.stringify({ $schema: "global" })),
              fs.writeFile(path.join(root, "openlegion.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(path.join(parent, "openlegion.jsonc"), JSON.stringify({ $schema: "parent" })),
              fs.writeFile(path.join(directory, "config.json"), JSON.stringify({ $schema: "directory" })),
              fs.writeFile(path.join(root, ".openlegion", "openlegion.json"), JSON.stringify({ $schema: "root-dot" })),
              fs.writeFile(
                path.join(directory, ".openlegion", "openlegion.jsonc"),
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(entries.filter((entry) => entry.type === "directory").map((entry) => entry.path)).toEqual([
              ...
              AbsolutePath.make(path.join(root, ".openlegion")),
              AbsolutePath.make(path.join(directory, ".openlegion")),
            ])
