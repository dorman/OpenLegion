...
import { and, Database, eq, isNull, lt, or, sql } from "@openlegion-ai/console-core/drizzle/index.js"
import { KeyTable } from "@openlegion-ai/console-core/schema/key.sql.js"
import { BillingTable, LiteTable, SubscriptionTable, UsageTable } from "@openlegion-ai/console-core/schema/billing.sql.js"
import { centsToMicroCents } from "@openlegion-ai/console-core/util/price.js"
import { getMonthlyBounds, getWeekBounds } from "@openlegion-ai/console-core/util/date.js"
import { Identifier } from "@openlegion-ai/console-core/identifier.js"
import { Billing } from "@openlegion-ai/console-core/billing.js"
import { Actor } from "@openlegion-ai/console-core/actor.js"
import { WorkspaceTable } from "@openlegion-ai/console-core/schema/workspace.sql.js"
import { ZenData } from "@openlegion-ai/console-core/model.js"
...
import { ProviderTable } from "@openlegion-ai/console-core/schema/provider.sql.js"
...
import { Resource } from "@openlegion-ai/console-resource"
...
export async function handler(
  ...
  try {
    ...
    const sessionId = input.request.headers.get("x-openlegion-session") ?? ""
    const requestId = input.request.headers.get("x-openlegion-request") ?? ""
    const ocClient = input.request.headers.get("x-openlegion-client") ?? ""
    ...
    const retriableRequest = async (retry: RetryOptions = { excludeProviders: [], retryCount: 0 }) => {
      ...
      const res = await fetchWith429Retry(reqUrl, {
        ...
        headers: (() => {
          ...
          headers.delete("x-openlegion-request")
          headers.delete("x-openlegion-session")
          headers.delete("x-openlegion-project")
          headers.delete("x-openlegion-client")
          return headers
        })(),
        body: reqBody,
      })
  ...
  function validateBilling(authInfo: AuthInfo, modelInfo: ModelInfo): BillingSource {
    ...
    if (opts.modelList === "lite" && authInfo.billing.lite && authInfo.lite) {
      try {
        const consoleGoUrl = `https://openlegion.dev/workspace/${authInfo.workspaceID}/go`
    ...
    const billingUrl = `https://openlegion.dev/workspace/${authInfo.workspaceID}/billing`
    const membersUrl = `https://openlegion.dev/workspace/${authInfo.workspaceID}/members`
