...
- Author package entrypoint is `@openlegion-ai/plugin/tui`.
...
```json
{
  "$schema": "https://openlegion.dev/tui.json",
  "theme": "smoke-theme",
  "leader_timeout": 2000,
  "keybinds": {
    "leader": "ctrl+x",
  ...
  "plugin": ["@acme/openlegion-plugin@1.2.3", ["./plugins/demo.tsx", { "label": "demo" }]],
  "plugin_enabled": {
    "acme.demo": false
  },
  "attention": {
...
- Import types from `@openlegion-ai/plugin/tui`.
- `@openlegion-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@openlegion-ai/plugin/tui"
...
- The value is a semver range checked against the running OpenLegion version.
- If the range is not satisfied, the plugin is skipped with a warning and a session error.
- If `engines.openlegion` is absent, no check is performed (backward compatible).
...
- `openlegion plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `openlegion plug <module>`.
- `-g` / `--global` writes into the global config dir.
- Local installs resolve target dir inside `patchPluginConfig`.
- For local scope, path is `<worktree>/.openlegion` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.openlegion`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.openlegion`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.openlegion` dir or `OPENLEGION_CONFIG_DIR`, OpenLegion installs `@openlegion-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@openlegion-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus OpenLegion's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
OpenLegion registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@openlegion-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"openlegion"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `openlegion.default`.
...
- Local plugins persist installed themes under the local `.openlegion/themes` area near the plugin config source.
...
- `--pure` / `OPENLEGION_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.openlegion/plugins/tui-smoke.tsx`
- Local vim plugin: `.openlegion/plugins/tui-vim.tsx`
- Local smoke config: `.openlegion/tui.json`
- Local smoke theme: `.openlegion/plugins/smoke-theme.json`
