...
Trim `packages/openlegion/src/server/routes/instance/httpapi/public.ts` until OpenAPI generation is mostly a direct projection of the `HttpApi` route declarations, without breaking the generated SDK surface.
...
- `[x]` Add or extend `packages/openlegion/test/server/httpapi-query-schema-drift.test.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/openlegion`.
- `[x]` `bun typecheck` from `packages/openlegion`.

### PR 2: Delete Spec-Only Workspace Query Injection

- `[x]` Edit `packages/openlegion/src/server/routes/instance/httpapi/public.ts`.
...
- `[x]` `bun test --timeout 5000 test/server/httpapi-query-schema-drift.test.ts` from `packages/openlegion`.
- `[x]` `bun dev generate > /tmp/openlegion-openapi.json` from `packages/openlegion`.
...
- Edit `packages/openlegion/src/server/routes/instance/httpapi/public.ts`.
...
- `bun dev generate > /tmp/openlegion-openapi.json` from `packages/openlegion`.
...
- Check source schemas in files like `packages/openlegion/src/session/schema.ts`, `packages/openlegion/src/permission/schema.ts`, and pty schema definitions.
...
- Edit route group files under `packages/openlegion/src/server/routes/instance/httpapi/groups/`.
- Replace SDK-visible `HttpApiError.BadRequest` / `HttpApiError.NotFound` with explicit error schemas from `packages/openlegion/src/server/routes/instance/httpapi/errors.ts` or add new ones there.
