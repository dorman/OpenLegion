...
Delete `packages/openlegion/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/openlegion/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/openlegion/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/openlegion`
- TUI plugin loader tests in `packages/openlegion` if runtime plugin API wiring changed
