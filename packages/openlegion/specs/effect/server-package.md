...
Practical reference for a future `packages/server` split after the openlegion
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/openlegion`.
...
Until enough shared service code lives outside `packages/openlegion`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/openlegion`

It should not import `packages/openlegion` services while `packages/openlegion`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on openlegion-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/openlegion` implementation details.
