...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.OPENLEGION_TEST ?? "")'), {
      env: {
        OPENLEGION_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "OPENLEGION_TEST_SHELL"], {
      shell: true,
      env: {
        OPENLEGION_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("OPENLEGION_TEST_SHELL=ok")
  })
