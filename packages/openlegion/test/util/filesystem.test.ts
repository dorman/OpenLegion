...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "openlegion.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "openlegion.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openlegion.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "openlegion.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["openlegion.json", "openlegion.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "openlegion.json"),
        path.join(project, "openlegion.jsonc"),
      ])
    })
  })
