...
import { SessionV1 } from "@openlegion-ai/core/v1/session"
import { FSUtil } from "@openlegion-ai/core/fs-util"
import { ModelsDev } from "@openlegion-ai/core/models-dev"
import { LocationServiceMap } from "@openlegion-ai/core/location-layer"
import { HttpRecorder, Redactor } from "@openlegion-ai/http-recorder"
...
import { LLMEvent, LLMResponse } from "@openlegion-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor } from "@openlegion-ai/llm/route"
import { Env } from "@/env"
...
const zenURL = (connection: string) => `https://console.openlegion.dev/proxy/connections/${connection}/v1`
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.OPENLEGION_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["openlegion", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENLEGION_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("OPENLEGION_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["openlegion", "native", "oauth", "tool-loop"],
  ...
  {
    id: "openlegion-proxy",
    name: "OpenLegion proxy",
    providerID: ProviderV2.ID.openlegion,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["openlegion", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.OPENLEGION_RECORD_CONSOLE_TOKEN && process.env.OPENLEGION_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openlegion,
        name: "OpenLegion Zen",
        env: ["OPENLEGION_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.OPENLEGION_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.OPENLEGION_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.OPENLEGION_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["openlegion", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("OPENLEGION_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("OPENLEGION_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("OPENLEGION_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set OPENLEGION_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  // Only the HTTP client is recorded; RequestExecutor and the openlegion LLM stack remain real.
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "openlegion.json"),
      JSON.stringify({ $schema: "https://openlegion.dev/config.json", ...scenario.config(model) }),
    ),
  )
