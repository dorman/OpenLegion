...
import { ProjectCopy } from "@openlegion-ai/core/project/copy"
...
describe("Project.fromDirectory", () => {
  it.live("should handle git repository with no commits", () =>
    Effect.gen(function* () {
      ...
      const openlegionFile = path.join(tmp, ".git", "openlegion")
      expect(yield* Effect.promise(() => Bun.file(openlegionFile).exists())).toBe(false)
    }),
  )
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      const cache = path.join(tmp, ".git", "openlegion")
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      const correctCache = path.join(barePath, "openlegion")
      const wrongCache = path.join(parentDir, ".git", "openlegion")
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      const cacheA = path.join(bareA, "openlegion")
      const cacheB = path.join(bareB, "openlegion")
      const wrongCache = path.join(parentDir, ".git", "openlegion")
