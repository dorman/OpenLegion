 ...
 *
 * The script intentionally isolates `OPENLEGION_DB` before importing modules that touch
...
void (await import("@openlegion-ai/core/util/log")).init({ print: false })
...
const scenarios: Scenario[] = [
  ...
  http.protected
    ...
    .seeded(() =>
      Effect.promise(() =>
        Bun.write(
          path.join(exerciseConfigDirectory, "openlegion.jsonc"),
      ...
      (body) =>
        Effect.gen(function* () {
          ...
          const text = yield* Effect.promise(() =>
            Bun.file(path.join(exerciseConfigDirectory, "openlegion.jsonc")).text(),
          )
  ...
  http.protected.get("/path", "path.get").json(200, (body, ctx) => {
    object(body)
    check(body.directory === ctx.directory, "directory should resolve from x-openlegion-directory")
    check(body.worktree === ctx.directory, "worktree should resolve from x-openlegion-directory")
  }),
  ...
  http.protected
    ...
    .at((ctx) => ({
      path: `/experimental/tool?${new URLSearchParams({ provider: "openlegion", model: "test" })}`,
      headers: ctx.headers(),
    }))
    .json(200, array, "status"),
  ...
  http.protected
    ...
    .status(400),
  http.protected
    .get("/experimental/container", "container.list")
    .at((ctx) => ({
      path: "/experimental/container",
