...
import { ProjectTable } from "@openlegion-ai/core/project/sql"
import { AbsolutePath } from "@openlegion-ai/core/schema"
import { Session as SessionNs } from "@/session/session"
import { SessionID } from "@/session/schema"
import { SessionTable } from "@openlegion-ai/core/session/sql"
import { EventSequenceTable } from "@openlegion-ai/core/event/sql"
...
import { WorkspaceV2 } from "@openlegion-ai/core/workspace"
import { WorkspaceTable } from "@openlegion-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  OPENLEGION_AUTH_CONTENT: process.env.OPENLEGION_AUTH_CONTENT,
  OPENLEGION_EXPERIMENTAL_WORKSPACES: process.env.OPENLEGION_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.OPENLEGION_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@openlegion.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.OPENLEGION_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        process.env.OTEL_EXPORTER_OTLP_HEADERS = "authorization=otel"
        process.env.OTEL_EXPORTER_OTLP_ENDPOINT = "https://otel.test"
        process.env.OTEL_RESOURCE_ATTRIBUTES = "service.name=openlegion-test"
        ...
        expect(JSON.parse(recorded.calls.create[0].env.OPENLEGION_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.OPENLEGION_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.OPENLEGION_EXPERIMENTAL_WORKSPACES).toBe("true")
        ...
        expect(recorded.calls.create[0].env.OTEL_RESOURCE_ATTRIBUTES).toBe("service.name=openlegion-test")
