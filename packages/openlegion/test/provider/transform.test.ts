    ...
    const msgs = [
      ...
      { role: "assistant", content: text("assistant string") },
      {
        ...
        content: [
          ...
          { type: "tool-call", toolCallId: "call-1", toolName: "Read", input: { filePath: ".openlegion/tool/emoji.ts" } },
          {
            type: "tool-result",
            toolCallId: "call-2",
            toolName: "Read",
...
describe("ProviderTransform.message - strip openai metadata when store=false", () => {
  ...
  test("preserves metadata using providerID key when store is false", () => {
    const openlegionModel = {
      ...openaiModel,
      providerID: "openlegion",
      api: {
        id: "openlegion-test",
        url: "https://api.openlegion.dev",
        npm: "@ai-sdk/openai-compatible",
      },
    }
    const msgs = [
    ...
    const result = ProviderTransform.message(msgs, openlegionModel, { store: false }) as any[]

    expect(result[0].content[0].providerOptions?.openlegion?.itemId).toBe("msg_123")
    expect(result[0].content[0].providerOptions?.openlegion?.otherOption).toBe("value")
  })
  ...
  test("preserves itemId across all providerOptions keys", () => {
    ...
    const msgs = [
      {
        ...
        providerOptions: {
          ...
          openlegion: { itemId: "msg_openlegion" },
        ...
        content: [
          {
            ...
            providerOptions: {
              ...
              openlegion: { itemId: "msg_openlegion_part" },
    ...
    expect(result[0].providerOptions?.openlegion?.itemId).toBe("msg_openlegion")
    expect(result[0].providerOptions?.extra?.itemId).toBe("msg_extra")
    expect(result[0].content[0].providerOptions?.openai?.itemId).toBe("msg_openai_part")
    expect(result[0].content[0].providerOptions?.openlegion?.itemId).toBe("msg_openlegion_part")
