...
import { Database } from "@openlegion-ai/core/database/database"
...
const configLayer = TestConfig.layer({
  directories: () => InstanceState.directory.pipe(Effect.map((dir) => [path.join(dir, ".openlegion")])),
})
...
describe("tool.registry", () => {
  ...
  it.instance("loads tools from .openlegion/tool (singular)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const openlegion = path.join(test.directory, ".openlegion")
      const tool = path.join(openlegion, "tool")
  ...
  it.instance("ignores non-tool exports in .openlegion/tool files", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tool = path.join(test.directory, ".openlegion", "tool")
      yield* Effect.promise(() => fs.mkdir(tool, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  // came in through `plugin.list()` — `oh-my-openlegion` was registering a tool
  ...
  it.instance("loads tools from .openlegion/tools (plural)", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const openlegion = path.join(test.directory, ".openlegion")
      const tools = path.join(openlegion, "tools")
  ...
  it.instance("loads Zod-schema custom tools with JSON Schema and validation", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const customTools = path.join(test.directory, ".openlegion", "tools")
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance
        const openlegion = path.join(test.directory, ".openlegion")
        const customTools = path.join(openlegion, "tools")
        const plugin = path.join(openlegion, "node_modules", "@openlegion-ai", "plugin")
        yield* Effect.promise(() => fs.mkdir(path.join(plugin, "dist"), { recursive: true }))
        yield* Effect.promise(() => fs.mkdir(customTools, { recursive: true }))
        yield* Effect.promise(() =>
          fs.cp(path.dirname(fileURLToPath(import.meta.resolve("zod"))), path.join(openlegion, "node_modules", "zod"), {
            dereference: true,
            recursive: true,
          }),
        )
        yield* Effect.promise(() =>
          Bun.write(
            ...
            JSON.stringify({ name: "@openlegion-ai/plugin", type: "module", exports: { ".": "./dist/index.js" } }),
          ),
        )
        yield* Effect.promise(() =>
          Bun.write(
        ...
        yield* Effect.promise(() =>
          Bun.write(
            ...
            [
              'import { tool } from "@openlegion-ai/plugin"',
              "export default tool({",
  ...
  it.instance("loads legacy JSON-schema-shaped custom tools with wire schema", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance
      const tools = path.join(test.directory, ".openlegion", "tools")
      yield* Effect.promise(() => fs.mkdir(tools, { recursive: true }))
      yield* Effect.promise(() =>
        Bun.write(
  ...
  it.instance("loads tools with external dependencies without crashing", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(openlegion, "package.json"),
          JSON.stringify({
            name: "custom-tools",
            dependencies: {
              "@openlegion-ai/plugin": "^0.0.0",
              cowsay: "^1.6.0",
            },
          }),
        ),
      ...
      yield* Effect.promise(() =>
        Bun.write(
          path.join(openlegion, "package-lock.json"),
          JSON.stringify({
            name: "custom-tools",
            lockfileVersion: 3,
            packages: {
      ...
      const cowsay = path.join(openlegion, "node_modules", "cowsay")
