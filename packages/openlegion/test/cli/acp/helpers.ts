...
export function createAcpClient(input: Pick<CliFixture, "openlegion">, env?: Record<string, string>) {
  return Effect.gen(function* () {
    return createJsonRpcAcpClient(yield* input.openlegion.acp(env ? { env } : undefined))
  })
}
  ...
  return Effect.gen(function* () {
    return expectOk(
      yield* acp.request<InitializeResponse>("initialize", {
        ...
        clientInfo: { name: "openlegion-local-acp", version: "0.1.0" },
      }),
    )
  })
}
