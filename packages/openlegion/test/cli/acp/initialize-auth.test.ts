...
describe("openlegion acp initialize/auth subprocess", () => {
  cliIt.live(
    "initialize responds with capabilities",
    ({ openlegion }) =>
      Effect.gen(function* () {
        const initialized = yield* initialize(yield* createAcpClient({ openlegion }))
        ...
        expect(initialized.agentInfo?.name).toBe("OpenLegion")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ openlegion })
        const initialized = yield* initialize(acp)

        expect(initialized.authMethods?.[0]?.id).toBe("openlegion-login")
        expect(initialized.authMethods?.[0]?._meta?.["terminal-auth"]).toBeDefined()
        expect(yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "openlegion-login" })).toMatchObject({
          result: {},
        })
        ...
        expect(JSON.stringify(rejected.error)).not.toContain(process.env.OPENLEGION_AUTH_CONTENT ?? "not-present")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        ...
        expect(initialized.result?.authMethods?.[0]?.id).toBe("openlegion-login")
