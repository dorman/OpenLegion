...
describe("openlegion read-only commands (smoke)", () => {
  // `mcp list` reads MCP server config and pings each one. With the empty
  // OPENLEGION_CONFIG_CONTENT={} we provide, no servers should be configured
  // and the command should report that cleanly.
  cliIt.live(
    "mcp list: exits 0",
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["mcp", "list"])
        openlegion.expectExit(r, 0, "mcp list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["providers", "list"])
        openlegion.expectExit(r, 0, "providers list")
  ...
  cliIt.live(
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["models"])
        openlegion.expectExit(r, 0, "models")
  ...
  cliIt.live(
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["agent", "list"])
        openlegion.expectExit(r, 0, "agent list")
      }),
    60_000,
  )

  // `session list` reads the session DB. Fresh OPENLEGION_TEST_HOME means
  // empty DB. Exit 0 with no sessions.
  cliIt.live(
    "session list: exits 0",
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["session", "list"])
        openlegion.expectExit(r, 0, "session list")
      }),
    60_000,
  )
  ...
  cliIt.live(
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["stats"])
        openlegion.expectExit(r, 0, "stats")
      }),
    60_000,
  )
  ...
  // in production it'd be a path under OPENLEGION_TEST_HOME / XDG_DATA_HOME.
  // Accept either form — both prove the resolver ran without crashing.
  cliIt.live(
    "db path: exits 0 and prints a path or :memory:",
    ({ openlegion }) =>
      Effect.gen(function* () {
        const r = yield* openlegion.spawn(["db", "path"])
        openlegion.expectExit(r, 0, "db path")
