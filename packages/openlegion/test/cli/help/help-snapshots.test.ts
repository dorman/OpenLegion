...
// terminal sizes. The default openlegion tui command is excluded —
// `openlegion --help` includes an ASCII banner that pulls in the install
...
// Top-level commands. Order matches what `openlegion --help` prints today;
...
// top-level help on `--help` and exits 1; not a real openlegion command.
const TOP_LEVEL = [
  "acp",
  "mcp",
  "attach",
...
describe("openlegion CLI help-text snapshots", () => {
    ...
    ({ openlegion }) =>
      Effect.gen(function* () {
        ...
        const [failures, results] = yield* Effect.partition(
          ...
          (argv) =>
            Effect.gen(function* () {
              const result = yield* openlegion.spawn([...argv, "--help"], { env: SNAPSHOT_ENV })
              if (result.exitCode !== 0) {
                return yield* Effect.fail(`openlegion ${argv.join(" ")}: exit ${result.exitCode}`)
              }
              return { argv, result }
            }),
          { concurrency: 8 },
        ...
        for (const { argv, result } of results) {
          ...
          expect(normalize(result.stderr)).toMatchSnapshot(`openlegion ${argv.join(" ")} --help`)
        }
