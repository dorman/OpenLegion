// Subprocess test harness for the openlegion CLI. Spawns the real binary against
...
//
// Configuration flows through openlegion's built-in test affordances:
//   - OPENLEGION_CONFIG_CONTENT      : provider config inline, no files to find
//   - OPENLEGION_TEST_HOME           : pins os.homedir() → tmpdir
//   - OPENLEGION_DISABLE_PROJECT_CONFIG : skip walking up for openlegion.json
//   - OPENLEGION_PURE                : skip external plugin discovery + install
//   - OPENLEGION_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
// Plus HOME / XDG_* pointing at the tmpdir for belt-and-suspenders isolation.
//
// Today only `openlegion.run` is fully wired. The shape supports adding more
// builders (`openlegion.serve(opts)`, `openlegion.acp(opts)`, `openlegion.auth(...)`)
...
const openlegionRoot = path.resolve(import.meta.dir, "../../")
const cliEntry = path.join(openlegionRoot, "src/index.ts")
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    OPENLEGION_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    OPENLEGION_CONFIG_CONTENT: configJson,
    OPENLEGION_DISABLE_PROJECT_CONFIG: "1",
    OPENLEGION_PURE: "1",
    OPENLEGION_DISABLE_AUTOUPDATE: "1",
    OPENLEGION_DISABLE_AUTOCOMPACT: "1",
    OPENLEGION_DISABLE_MODELS_FETCH: "1",
    OPENLEGION_AUTH_CONTENT: "{}",
  }
}
...
// Typed equivalent of constructing argv for `openlegion run`. New flags should
...
// `openlegion serve` is a long-lived process — it never exits on its own.
...
// `openlegion acp` speaks newline-delimited JSON-RPC over stdin/stdout. It is
...
export type OpencodeCli = {
  ...
  // Spawn `openlegion serve` and wait until it's listening. Long-lived: the
  ...
  // Spawn `openlegion acp` and return a duplex JSON-RPC handle. Long-lived:
...
export type CliFixture = {
  ...
  readonly openlegion: OpencodeCli
}
...
export function withCliFixture<A, E>(
  ...
  return Effect.gen(function* () {
    ...
    const spawn = Effect.fn("openlegion.spawn")(function* (args: string[], opts?: SpawnOpts) {
    ...
    const serve = Effect.fn("openlegion.serve")(function* (opts?: ServeOpts) {
      const argv = ["serve"]
      ...
      //   "openlegion server listening on http://<host>:<port>"
      ...
      const match = yield* Deferred.await(readyDeferred).pipe(
        Effect.timeoutOrElse({
          ...
          orElse: () =>
            Effect.fail(
              new Error(
                `openlegion serve did not become ready within ${readyTimeoutMs}ms\n` +
    ...
    const acp = Effect.fn("openlegion.acp")(function* (opts?: AcpOpts) {
      const argv = ["acp"]
    ...
    const openlegion: OpencodeCli = { run, serve, acp, spawn, expectExit, parseJsonEvents }

    return yield* fn({ llm, home, openlegion })
    // FetchHttpClient is provided so test bodies can `yield* HttpClient.HttpClient`
    // and hit endpoints on `openlegion.serve()` without rolling their own fetch.
  }).pipe(
    Effect.provide(
...
function expectExit(result: RunResult, expected: number, label = "openlegion") {
...
// every call site. Use this for any test that needs the openlegion CLI fixture.
//
...
// resources (e.g. `openlegion.serve`) without an extra `Effect.scoped` wrapper —
