...
it.instance("plan agent denies edits except .openlegion/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".openlegion/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".openlegion", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.OPENLEGION_TEST_HOME
      process.env.OPENLEGION_TEST_HOME = test.directory
