...
import type { Message, OpencodeClient, SessionMessageResponse } from "@openlegion-ai/sdk/v2"
...
export const AuthMethodID = "openlegion-login"
...
export class Service extends Context.Service<Service, Interface>()("@openlegion/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `openlegion auth login` in the terminal",
      name: "Login with openlegion",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, openlegion provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const openlegionProvider = providers[ProviderV2.ID.make("openlegion")]
  const openlegionModel = openlegionProvider ? Provider.sort(Object.values(openlegionProvider.models))[0] : undefined
  if (openlegionProvider && openlegionModel) return { providerID: openlegionProvider.id, modelID: openlegionModel.id }
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "OpenLegion service failure", service })
}
