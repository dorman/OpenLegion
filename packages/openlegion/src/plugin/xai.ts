...
export function buildAuthorizeUrl(
  ...
  // clients. `referrer=openlegion` lets xAI attribute openlegion-originated
...
<html>
  <head>
    <title>OpenLegion - xAI Authorization Successful</title>
    <style>
      body {
        font-family:
          system-ui,
  ...
  <body>
    <div class="container">
      ...
      <p>You can close this window and return to OpenLegion.</p>
    </div>
    <script>
...
<html>
  <head>
    <title>OpenLegion - xAI Authorization Failed</title>
    <style>
      body {
        font-family:
          system-ui,
...
async function startOAuthServer(): Promise<{ port: number; redirectUri: string }> {
  ...
  await new Promise<void>((resolve, reject) => {
    ...
    server.listen(OAUTH_PORT, OAUTH_HOST, () => {
      ...
      // behavior and crash the entire openlegion process. Matches the silent-
      ...
      async loader(getAuth) {
        ...
        return {
          ...
          async fetch(requestInput: RequestInfo | URL, init?: RequestInit) {
            ...
            headers.set("User-Agent", `openlegion/${InstallationVersion}`)
