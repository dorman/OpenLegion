...
export async function checkPluginCompatibility(target: string, openlegionVersion: string, pkg?: PluginPackage) {
  if (!semver.valid(openlegionVersion) || semver.major(openlegionVersion) === 0) return
  const hit = pkg ?? (await readPluginPackage(target).catch(() => undefined))
  if (!hit) return
  const engines = hit.json.engines
  if (!isRecord(engines)) return
  const range = engines.openlegion
  if (typeof range !== "string") return
  if (!semver.satisfies(openlegionVersion, range)) {
    throw new Error(`Plugin requires openlegion ${range} but running ${openlegionVersion}`)
  }
}
