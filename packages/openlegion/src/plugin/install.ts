...
export type PatchDeps = {
  ...
  files: (dir: string, name: "openlegion" | "tui") => string[]
}
...
function patchDir(input: PatchInput) {
  ...
  return path.join(root, ".openlegion")
}

function patchName(kind: Kind): "openlegion" | "tui" {
  if (kind === "server") return "openlegion"
  return "tui"
}
