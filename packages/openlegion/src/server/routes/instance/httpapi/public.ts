...
import { OpenLegionHttpApi } from "./api"
...
const LegacyComponentDescriptions: Record<string, string> = {
  LogLevel: "Log level",
  ServerConfig: "Server configuration for openlegion serve and web commands",
...
function fixSelfReferencingComponents(spec: OpenApiSpec) {
  ...
  const raw: OpenApiSpec = OpenApi.fromApi(OpenLegionHttpApi)
...
export const PublicApi = OpenLegionHttpApi.annotateMerge(
  OpenApi.annotations({
    title: "openlegion",
    version: "1.0.0",
    description: "openlegion api",
