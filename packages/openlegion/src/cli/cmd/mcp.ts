...
import { ConfigMCPV1 } from "@openlegion-ai/core/v1/config/mcp"
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    if (servers.length === 0) {
      ...
      prompts.outro("Add servers with: openlegion mcp add")
      return
    }
...
export const McpAuthCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth")(function* (args) {
    ...
    if (servers.length === 0) {
      ...
      prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in openlegion.json:")
      prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .openlegion/ subdirectory too)
  const candidates = [path.join(baseDir, "openlegion.json"), path.join(baseDir, "openlegion.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".openlegion", "openlegion.json"), path.join(baseDir, ".openlegion", "openlegion.jsonc"))
  }
  ...
  // Default to openlegion.json if none exist
  return candidates[0]
}
    ...
    yield* Effect.promise(async () => {
      ...
      if (type === "local") {
        const command = await prompts.text({
          ...
          placeholder: "e.g., openlegion x @modelcontextprotocol/server-filesystem",
          validate: (x) => (x && x.length > 0 ? undefined : "Required"),
        })
        ...
        const response = await fetch(serverConfig.url, {
          ...
          body: JSON.stringify({
            ...
            params: {
              ...
              capabilities: {},
              clientInfo: { name: "openlegion-debug", version: InstallationVersion },
            },
            id: 1,
          }),
        })
