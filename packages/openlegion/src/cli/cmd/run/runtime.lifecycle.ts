...
export async function createRuntimeLifecycle(input: LifecycleInput): Promise<Lifecycle> {
  return withRunSpan(
    "RunLifecycle.boot",
    {
      "openlegion.agent.name": input.agent,
      "openlegion.directory": input.directory,
      "openlegion.first": input.first,
      "openlegion.model.provider": input.model?.providerID,
      "openlegion.model.id": input.model?.modelID,
      "openlegion.model.variant": input.variant,
      ...
      try {
        ...
        const close = async (next: {
          ...
          return withRunSpan(
            "RunLifecycle.close",
            {
              "openlegion.show_exit": next.showExit,
