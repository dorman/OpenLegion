...
import { createOpencodeClient } from "@openlegion-ai/sdk/v2"
...
async function runInteractiveRuntime(input: RunRuntimeInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "openlegion.mode": input.resolveSession ? "local" : "attach",
      "openlegion.initial_input": !!input.initialInput,
      "openlegion.demo": input.demo,
    },
    async (span) => {
      ...
      setRunSpanAttributes(span, {
        "openlegion.directory": ctx.directory,
        "openlegion.resume": ctx.resume === true,
        "openlegion.agent.name": state.agent,
        "openlegion.model.provider": state.model?.providerID,
        "openlegion.model.id": state.model?.modelID,
        "openlegion.model.variant": state.activeVariant,
      ...
      const shell = await createRuntimeLifecycle({
        ...
        onModelSelect: async (model) => {
          ...
          setRunSpanAttributes(span, {
            "openlegion.model.provider": model.providerID,
            "openlegion.model.id": model.modelID,
            "openlegion.model.variant": state.activeVariant,
          })
          return {
          ...
          run: async (prompt, signal) => {
            ...
            return withRunSpan(
              "RunInteractive.turn",
              {
                ...
                "openlegion.model.id": state.model?.modelID,
                "openlegion.model.variant": state.activeVariant,
                "openlegion.prompt.chars": prompt.text.length,
                "openlegion.prompt.parts": prompt.parts.length,
                "openlegion.prompt.include_files": includeFiles,
                "openlegion.prompt.file_parts": includeFiles ? input.files.length : 0,
                "session.id": state.sessionID || undefined,
              },
              async (span) => {
                try {
...
export async function runInteractiveLocalMode(input: RunLocalInput): Promise<void> {
  return withRunSpan(
    ...
    {
      "openlegion.directory": input.directory,
      "openlegion.initial_input": !!input.initialInput,
      "openlegion.demo": input.demo,
    },
    async () => {
      const sdk = createOpencodeClient({
        baseUrl: "http://openlegion.internal",
        fetch: input.fetch,
