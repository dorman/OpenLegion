...
const TUI_SCHEMA_URL = "https://openlegion.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from openlegion.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const openlegion = await openlegionFiles(input)
  for (const file of openlegion) {
...
async function openlegionFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "openlegion"),
    ...(await Filesystem.findUp(["openlegion.json", "openlegion.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "openlegion"))
  }
  if (Flag.OPENLEGION_CONFIG) files.push(Flag.OPENLEGION_CONFIG)
