...
import openlegion from "./theme/openlegion.json" with { type: "json" }
...
import type { TuiThemeCurrent } from "@openlegion-ai/plugin/tui"
  ...
  init: (props: { mode: "dark" | "light" }) => {
    ...
    setStore(
      produce((draft) => {
        ...
        draft.mode = mode
        draft.lock = lock
        const active = config.theme ?? kv.get("theme", "openlegion")
        draft.active = typeof active === "string" ? active : "openlegion"
        draft.ready = false
      }),
    )
    ...
    const values = createMemo(() => {
      ...
      return resolveTheme(store.themes.openlegion, store.mode)
    })
