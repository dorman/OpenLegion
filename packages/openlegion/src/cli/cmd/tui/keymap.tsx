...
export const OPENLEGION_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const OPENLEGION_MODE_KEY = "openlegion.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(OPENLEGION_MODE_KEY, OPENLEGION_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(OPENLEGION_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(OPENLEGION_MODE_KEY, stack.at(-1)?.mode ?? OPENLEGION_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? OPENLEGION_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(OPENLEGION_MODE_KEY, undefined)
