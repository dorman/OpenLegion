...
import { Flock } from "@openlegion-ai/core/util/flock"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".openlegion"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".openlegion", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".openlegion", "tui.json"),
  }
}
...
async function load(input: { api: Api; config: TuiConfig.Resolved; dispose?: () => void; disposeTimeoutMs?: number }) {
  ...
  try {
    ...
    const records = Flag.OPENLEGION_PURE ? [] : (config.plugin_origins ?? [])
    if (Flag.OPENLEGION_PURE && config.plugin_origins?.length) {
