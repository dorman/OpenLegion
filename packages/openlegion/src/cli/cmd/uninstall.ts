...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall openlegion and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall OpenLegion")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g openlegion-ai",
      pnpm: "pnpm uninstall -g openlegion-ai",
      bun: "bun remove -g openlegion-ai",
      yarn: "yarn global remove openlegion-ai",
      brew: "brew uninstall openlegion",
      choco: "choco uninstall openlegion",
      scoop: "scoop uninstall openlegion",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "openlegion", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
  ...
  UI.empty()
  prompts.log.success("Thank you for using OpenLegion!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# openlegion") || content.includes(".openlegion/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".openlegion/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".openlegion/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".openlegion"))
    ) {
      continue
    }
