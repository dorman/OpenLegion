import type { PermissionV1 } from "@openlegion-ai/core/v1/permission"
// CLI entry point for `openlegion run`.
//
// Handles three modes:
...
//      openlegion server and runs interactive mode against it.
//
...
import { createOpencodeClient, type OpencodeClient, type ToolPart } from "@openlegion-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run openlegion with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running openlegion server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to OPENLEGION_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENLEGION_SERVER_USERNAME or 'openlegion')",
      })
      .option("dir", {
        type: "string",
