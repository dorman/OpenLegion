...
import { tool as nativeTool, ToolFailure, type JsonSchema, type LLMEvent } from "@openlegion-ai/llm"
import type { LLMClientShape } from "@openlegion-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("openlegion"))
    return { type: "unsupported", reason: "provider is not openai, openlegion, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @openlegion-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains openlegion-owned. The native runtime only adapts
      // the @openlegion-ai/llm tool call back into the AI SDK Tool.execute shape.
      nativeTool({
