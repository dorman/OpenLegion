...
const MANAGED_PLIST_DOMAIN = "ai.openlegion.managed"

// Keys injected by macOS/MDM into the managed plist that are not OpenLegion config
...
function systemManagedConfigDir(): string {
  switch (process.platform) {
    case "darwin":
      return "/Library/Application Support/openlegion"
    case "win32":
      return path.join(process.env.ProgramData || "C:\\ProgramData", "openlegion")
    default:
      return "/etc/openlegion"
  }
}

export function managedConfigDir() {
  return process.env.OPENLEGION_TEST_MANAGED_CONFIG_DIR || systemManagedConfigDir()
}
