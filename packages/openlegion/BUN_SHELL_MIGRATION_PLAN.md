...
Replace runtime Bun shell template-tag usage in `packages/openlegion/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/openlegion/src` is removed except:
