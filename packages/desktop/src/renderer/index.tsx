...
import "./styles.css"
import { useTheme } from "@openlegion-ai/ui/theme"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.OPENLEGION_CHANNEL === "prod" &&
...
const deepLinkEvent = "openlegion:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__OPENLEGION__ ??= {}
  const pending = window.__OPENLEGION__.deepLinks ?? []
  window.__OPENLEGION__.deepLinks = [...pending, ...urls]
...
render(() => {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("openlegion.global.dat").getItem("language")
