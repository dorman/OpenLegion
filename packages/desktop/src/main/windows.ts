...
import type { DesktopTheme } from "@openlegion-ai/ui/theme/types"
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("OpenLegion is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
