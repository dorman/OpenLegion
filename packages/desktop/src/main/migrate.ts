...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.openlegion.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `openlegion.settings.dat` is special: it maps to the `openlegion.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"openlegion.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // openlegion.settings.dat → the electron settings store ("openlegion.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "openlegion.global.dat").
  const storeName = filename === "openlegion.settings.dat" ? "openlegion.settings" : filename
