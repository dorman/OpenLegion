    ...
    const result = {
      ...
      session: {
        ...
        async sync(sessionID: string) {
          ...
          const task = (async () => {
            const [session, fetched, todo, diff] = await Promise.all([
              ...
              produce((draft) => {
                ...
                infos.push(
                  ...currentMessages.filter(
                    (message) =>
                      (tracker.messages.has(message.id) || optimisticMessages.has(message.id)) &&
                ...
                for (const message of fetched.data ?? []) {
                  ...
                  if (message.parts.length === 0 && optimisticMessages.has(message.info.id) && currentParts.length > 0) {
