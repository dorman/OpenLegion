...
# パッケージマネージャー
npm i -g openlegion-ai@latest        # bun/pnpm/yarn でもOK
scoop install openlegion             # Windows
choco install openlegion             # Windows
brew install anomalyco/tap/openlegion # macOS と Linux（推奨。常に最新）
brew install openlegion              # macOS と Linux（公式 brew formula。更新頻度は低め）
sudo pacman -S openlegion            # Arch Linux (Stable)
paru -S openlegion-bin               # Arch Linux (Latest from AUR)
mise use -g openlegion               # どのOSでも
nix run nixpkgs#openlegion           # または github:dorman/OpenLegion で最新 dev ブランチ
```
...
OpenLegion はデスクトップアプリとしても利用できます。[releases page](https://github.com/dorman/OpenLegion/releases) から直接ダウンロードするか、[openlegion.dev/download](https://openlegion.dev/download) を利用してください。
...
1. `$OPENLEGION_INSTALL_DIR` - カスタムのインストールディレクトリ
2. `$XDG_BIN_DIR` - XDG Base Directory Specification に準拠したパス
3. `$HOME/bin` - 標準のユーザー用バイナリディレクトリ（存在する場合、または作成できる場合）
4. `$HOME/.openlegion/bin` - デフォルトのフォールバック
...
OpenLegion には組み込みの Agent が2つあり、`Tab` キーで切り替えられます。
...
[agents](https://openlegion.dev/docs/agents) の詳細はこちら。

### ドキュメント

OpenLegion の設定については [**ドキュメント**](https://openlegion.dev/docs) を参照してください。

### コントリビュート

OpenLegion に貢献したい場合は、Pull Request を送る前に [contributing docs](./CONTRIBUTING.md) を読んでください。

### OpenLegion の上に構築する

OpenLegion に関連するプロジェクトで、名前に "openlegion"（例: "openlegion-dashboard" や "openlegion-mobile"）を含める場合は、そのプロジェクトが OpenLegion チームによって作られたものではなく、いかなる形でも関係がないことを README に明記してください。

---

**コミュニティに参加** [Discord](https://discord.gg/openlegion) | [X.com](https://x.com/openlegion)
